...
fun <T : HttpRequest> T.withAuth(): T =
    this.apply {
      ...
      addHeader(
          BasicScheme()
              .authenticate(
                  UsernamePasswordCredentials(teamCityCI.userName, teamCityCI.password),
                  this,
                  null))
    }
...
object TeamCityClient {
  ...
  val artifactForPublishingDir: Path by lazy {
    GlobalPaths.instance.testsDirectory / "teamcity-artifacts-for-publish"
  }
  ...
  /**
   * [source] - source path of artifact [artifactPath] - new path (relative, where artifact will be
   * present) [artifactName] - name of artifact
   */
  fun publishTeamCityArtifacts(
    ...
    do {
      ...
      artifactDir =
          (artifactForPublishingDir / sanitizedArtifactPath / (sanitizedArtifactName + suffix))
              .normalize()
              .toAbsolutePath()
      nextSuffix++
    ...
    if (source.isDirectory()) {
      ...
      if (zipContent) {
        printTcArtifactsPublishMessage(
            "${artifactDir.toRealPath()}/** => $sanitizedArtifactPath/$sanitizedArtifactName$suffix.zip")
      } else {
        printTcArtifactsPublishMessage(
            "${artifactDir.toRealPath()}/** => $sanitizedArtifactPath$suffix")
      }
    } else {
      ...
      if (zipContent) {
        printTcArtifactsPublishMessage(
            "${tempFile.toRealPath()} => $sanitizedArtifactPath/${sanitizedArtifactName + suffix}.zip")
      } else {
