...
open class TeamCityCIServer(
  ...
  override fun publishArtifact(source: Path, artifactPath: String, artifactName: String) {
    TeamCityClient.publishTeamCityArtifacts(
        source = source, artifactPath = artifactPath, artifactName = artifactName)
  }
  ...
  val buildStartTime: String by lazy {
    if (buildId == LOCAL_RUN_ID) {
      ...
      TeamCityClient.get(fullUrl) { it.withAuth() }
          .fields()
          .asSequence()
          ...
          ?.let {
            runCatching {
                  ZonedDateTime.parse(it, DateTimeFormatter.ofPattern("yyyyMMdd'T'HHmmssX"))
  ...
  private val listOfPatternsWhichShouldBeIgnored by lazy {
    ...
    val patterns =
        mutableListOf(
            ...
            "Non-idempotent computation: it returns different results when invoked multiple times"
                .toRegex())
    ...
    val paramValue =
        if (impreciseNameMatch) {
          ...
          if (paramCandidates.size > 1)
              System.err.println(
                  "Found many parameters matching $name. Candidates: $paramCandidates")
  ...
  val buildId: String by lazy { getBuildParam("teamcity.build.id") ?: LOCAL_RUN_ID }
  ...
  private fun hasBooleanProperty(key: String, default: Boolean) =
      getBuildParam(key)?.equals("true", ignoreCase = true) ?: default
  ...
  companion object {
    ...
    fun String.processStringForTC(): String {
      // todo replace to intellij.platform.testFramework.util.teamcity.escapeStringForTeamCity when
      // module is published
    ...
    fun testSuiteStarted(suiteName: String, flowId: String) {
      println(
          "##teamcity[testSuiteStarted name='${suiteName.processStringForTC()}' flowId='$flowId']")
    }

    fun testSuiteFinished(suiteName: String, flowId: String) {
      println(
          "##teamcity[testSuiteFinished name='${suiteName.processStringForTC()}' flowId='$flowId']")
    }
