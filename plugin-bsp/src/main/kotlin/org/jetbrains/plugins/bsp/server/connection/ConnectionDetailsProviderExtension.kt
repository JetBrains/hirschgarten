 ...
 *
 * It should provide the latest available connection details so the client is always connected to
 * the latest server. For example if the connection file has changed after initial import of the
 * project, this extension allows the client to disconnect from the old server and connection to the
 * new one.
 *
...
public interface ConnectionDetailsProviderExtension : WithBuildToolId {
  /**
   * Method called only on the first opening of the project (so initial sync). It should be used to
   * run an initial configuration, e.g. show a wizard.
   *
   ...
   *
   * @return [true] if all the actions have succeeded and initial sync should continue; [false] if
   *   something has failed and initial sync should be terminated.
   */
   ...
   *
   * The method is called before each task executed on server. Client will disconnect from the old
   * server and connect to the new one if non-null value is returned.
   *
   * [currentConnectionDetails] set to [null] means that client is not aware of any connection
   * details (like during initial sync or after reopening the project)
   *
   * @return [null] if the newest available connection details are equal to
   *   [currentConnectionDetails]; otherwise new [BspConnectionDetails] if available
   */
...
/**
 * Java shim of [ConnectionDetailsProviderExtension], should be used only if coroutines are not
 * available, e.g. Java or Scala are used.
 */
...
private fun calculateJavaShimProviderInAdapter(
    buildToolId: BuildToolId
): ConnectionDetailsProviderExtension? =
    ConnectionDetailsProviderExtensionJavaShim.ep.withBuildToolId(buildToolId)?.let {
      ConnectionDetailsProviderExtensionAdapter(it)
    }
