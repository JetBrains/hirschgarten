...
public class BspProjectDirectoriesWorkspaceFileIndexContributor :
  ...
  override val entityClass: Class<BspProjectDirectoriesEntity> =
  ...
  private fun WorkspaceFileSetRegistrar.registerIncludedDirectories(
      entity: BspProjectDirectoriesEntity
  ) =
      entity.includedRoots.forEach {
        registerFileSet(
            root = it, kind = WorkspaceFileKind.CONTENT, entity = entity, customData = null)
      }

  private fun WorkspaceFileSetRegistrar.registerExcludedDirectories(
      entity: BspProjectDirectoriesEntity
  ) =
  ...
  private fun WorkspaceFileSetRegistrar.registerAllOtherDirectoriesAsExcluded(
      entity: BspProjectDirectoriesEntity
  ) =
  ...
  private fun VirtualFile.isNotIncludedInTheProject(entity: BspProjectDirectoriesEntity) =
      entity.includedRoots.mapNotNull { it.virtualFile }.none { it.isEqualOrParentOf(this) }
