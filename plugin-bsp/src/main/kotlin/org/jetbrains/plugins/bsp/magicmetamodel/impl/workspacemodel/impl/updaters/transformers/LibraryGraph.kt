...
internal class LibraryGraph(private val libraries: List<LibraryItem>) {
  ...
  fun calculateAllDependencies(
      ...
      if (includesTransitive) calculateAllTransitiveDependencies(target)
      else calculateDirectDependencies(target)
  ...
  private fun calculateAllTransitiveDependencies(target: BuildTarget): LibraryGraphDependencies {
    ...
    while (toVisit.isNotEmpty()) {
      ...
      if (currentNode !in visited) {
        ...
        if (currentNode.isCurrentNodeLibrary()) toVisit += graph[currentNode].orEmpty()
        visited += currentNode
              ...
              genericModuleInfo =
                  GenericModuleInfo(
                      ...
                      librariesDependencies =
                          listOf(IntermediateLibraryDependency(libraryName, true)),
                      modulesDependencies =
                          library.dependencies.map {
                            IntermediateModuleDependency(
                                libraryNameProvider(BuildTargetInfo(id = it)))
                          },
