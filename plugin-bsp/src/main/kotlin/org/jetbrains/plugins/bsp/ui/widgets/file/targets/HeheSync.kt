...
class HeheSync(private val file: VirtualFile) :
  SuspendableAction({ "sync file" }, BspPluginIcons.reload) {
  override suspend fun actionPerformed(project: Project, e: AnActionEvent) {
    val a = project.connection.runWithServer {
      s, c ->
      s.buildTargetInverseSources(InverseSourcesParams(TextDocumentIdentifier(file.url)))
    }.await()
    val syncScope = PartialProjectSync(targetsToSync = a.targets)
