...
internal class BspProjectModuleBuildTasksTracker :
    PersistentStateComponent<BspProjectModuleBuildTasksTracker> {
  ...
  companion object {
    ...
    /**
     * Assume that any tasks that satisfy BspProjectTaskRunner().canRun were run by
     * BspProjectTaskRunner
     */
    private fun ProjectTaskManager.Result.isRunByBsp() = anyTaskMatches { task, _ ->
      BspProjectTaskRunner().canRun(task)
    }

    private fun ProjectTaskManager.Result.isModuleBuildTaskRunSuccessfully() =
        anyTaskMatches { task, state ->
