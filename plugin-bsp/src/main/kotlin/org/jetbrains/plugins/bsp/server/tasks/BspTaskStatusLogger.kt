...
public class BspTaskStatusLogger<T>(
  ...
  public fun getResult(): T =
      taskFuture.reactToExceptionIn(cancelOn).catchBuildErrors().get().also {
  ...
  private fun finishBuildConsoleTaskWithProperResult(
      ...
      when (result.statusCode()) {
        StatusCode.OK ->
            bspBuildConsole.finishTask(uuid, BspPluginBundle.message("console.task.status.ok"))
        StatusCode.CANCELLED ->
            bspBuildConsole.finishTask(
                uuid, BspPluginBundle.message("console.task.status.cancelled"))
        StatusCode.ERROR ->
            bspBuildConsole.finishTask(
                uuid, BspPluginBundle.message("console.task.status.error"), FailureResultImpl())
        else ->
            bspBuildConsole.finishTask(uuid, BspPluginBundle.message("console.task.status.other"))
      }
