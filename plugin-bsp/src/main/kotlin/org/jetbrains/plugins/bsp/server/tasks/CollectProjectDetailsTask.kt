...
public class CollectProjectDetailsTask(project: Project, private val taskId: Any) :
  ...
  public suspend fun execute(name: String, cancelable: Boolean, buildProject: Boolean = false) {
    ...
    withContext(Dispatchers.Default) {
      coroutineJob = launch {
        try {
          withBackgroundProgress(project, name, cancelable) { doExecute(buildProject) }
  ...
  private suspend fun doExecute(buildProject: Boolean) {
    reportSequentialProgress { reporter ->
      ...
      reporter.indeterminateStep(
          text = BspPluginBundle.message("progress.bar.calculate.jdk.infos")) {
            ...
            uniqueJavaHomes.orEmpty().also {
              if (it.isNotEmpty())
                  ...
                  projectDetails.defaultJdkName =
                      SdkUtils.getProjectJdkOrMostRecentJdk(project)?.name
            }
          }

      if (BspFeatureFlags.isPythonSupportEnabled && pythonSdkGetterExtensionExists()) {
        reporter.indeterminateStep(
            text = BspPluginBundle.message("progress.bar.calculate.python.sdk.infos")) {
      ...
      if (BspFeatureFlags.isAndroidSupportEnabled && androidSdkGetterExtensionExists()) {
        reporter.indeterminateStep(
            text = BspPluginBundle.message("progress.bar.calculate.android.sdk.infos")) {
              calculateAllAndroidSdkInfosSubtask(projectDetails)
            }
      }

      reporter.sizedStep(
          workSize = 25, text = BspPluginBundle.message("progress.bar.update.internal.model")) {
            updateInternalModelSubtask(projectDetails)
          }

      reporter.sizedStep(
          workSize = 25, text = BspPluginBundle.message("progress.bar.post.processing")) {
  ...
  private fun collectModel(
        ...
        when {
          isCancellationException(e) ->
              bspSyncConsole.finishTask(
                  ...
                  result =
                      FailureResultImpl(
                          BspPluginBundle.message("console.task.exception.cancellation.message")),
              )
          ...
          isTimeoutException(e) ->
              bspSyncConsole.finishTask(
                  ...
                  result =
                      FailureResultImpl(
                          BspPluginBundle.message("console.task.exception.timeout.message")),
              )
    ...
    bspSyncConsole.finishSubtask(
        importSubtaskId, BspPluginBundle.message("console.task.model.collect.success"))
  ...
  private fun calculateAllScalaSdkInfos(projectDetails: ProjectDetails): Set<ScalaSdk> =
      projectDetails.targets.mapNotNull { createScalaSdk(it) }.toSet()

  private fun createScalaSdk(target: BuildTarget): ScalaSdk? =
      extractScalaBuildTarget(target)?.let {
        ScalaSdk(
  ...
  private fun createPythonSdk(
      ...
      extractPythonBuildTarget(target)?.let {
        if (it.interpreter != null && it.version != null)
            ...
            pythonSdkGetterExtension()?.getSystemSdk()?.let { sdk ->
              PythonSdk(
  ...
  private fun calculateAllPythonSdkInfos(projectDetails: ProjectDetails): Set<PythonSdk> {
    return projectDetails.targets
        .mapNotNull {
          createPythonSdk(
              it, projectDetails.dependenciesSources.filter { a -> a.target.uri == it.id.uri })
        }
        .toSet()
  }
  ...
  private fun calculateAllAndroidSdkInfos(projectDetails: ProjectDetails): Set<AndroidSdk> =
      projectDetails.targets.mapNotNull { createAndroidSdk(it) }.toSet()
          ...
          runInterruptible {
            ...
            val libraryModules =
                bspTracer.spanBuilder("create.library.modules.ms").use {
                  libraryGraph.createLibraryModules(
                      libraryNameProvider, projectDetails.defaultJdkName)
                }

            val targetIdToModuleDetails =
                bspTracer.spanBuilder("create.module.details.ms").use {
                  val transformer =
                      ProjectDetailsToModuleDetailsTransformer(projectDetails, libraryGraph)
                  projectDetails.targetIds.associateWith {
                    transformer.moduleDetailsForTargetId(it)
                  }
                }
            ...
            val targetIdToModuleEntitiesMap =
                bspTracer.spanBuilder("create.target.id.to.module.entities.map.ms").use {
                  val targetIdToTargetInfo =
                      projectDetails.targets.associate { it.id to it.toBuildTargetInfo() }
                      ...
                      TargetIdToModuleEntitiesMap(
                          ...
                          isAndroidSupportEnabled =
                              BspFeatureFlags.isAndroidSupportEnabled &&
                                  androidSdkGetterExtensionExists(),
                      )
            ...
            bspTracer.spanBuilder("load.modules.ms").use {
              ...
              workspaceModelUpdater.loadModules(
                  modulesToLoad + project.temporaryTargetUtils.getAllLibraryModules())
              workspaceModelUpdater.loadLibraries(project.temporaryTargetUtils.getAllLibraries())
              workspaceModelUpdater.loadDirectories(
                  projectDetails.directories, projectDetails.outputPathUris, virtualFileUrlManager)
            }
          }
        }
  }
  ...
  private fun WorkspaceModelUpdater.loadDirectories(
    ...
    val includedDirectories =
        directories.includedDirectories.map { it.toVirtualFileUrl(virtualFileUrlManager) }
    val excludedDirectories =
        directories.excludedDirectories.map { it.toVirtualFileUrl(virtualFileUrlManager) }
  ...
  private fun DirectoryItem.toVirtualFileUrl(
      virtualFileUrlManager: VirtualFileUrlManager
  ): VirtualFileUrl = virtualFileUrlManager.getOrCreateFromUrl(uri)
  ...
  private suspend fun addBspFetchedJdks() =
      withSubtask(
          "add-bsp-fetched-jdks", BspPluginBundle.message("console.task.model.add.fetched.jdks")) {
            bspTracer.spanBuilder("add.bsp.fetched.jdks.ms").useWithScope {
              uniqueJavaHomes?.forEach {
                SdkUtils.addJdkIfNeeded(projectName = project.name, javaHomeUri = it)
              }
            }
          }
  ...
  private suspend fun addPythonSdkIfNeeded(
      pythonSdk: PythonSdk,
      pythonSdkGetterExtension: PythonSdkGetterExtension
  ) {
    val sdk = runInterruptible {
      pythonSdkGetterExtension.getPythonSdk(
          pythonSdk, WorkspaceModel.getInstance(project).getVirtualFileUrlManager())
    }
  ...
  private suspend fun applyOnWorkspaceModel() {
    ...
    writeAction {
      ...
      if (!workspaceModelUpdated) {
        error(
            "Project model is not updated successfully. Try `reload` action to recalculate the project model.")
      }
    }
  }
...
public fun calculateProjectDetailsWithCapabilities(
  ...
  fun <Result> query(
      ...
      if (check)
          doQuery().reactToExceptionIn(cancelOn).catchSyncErrors(errorCallback).exceptionally {
  ...
  try {
    val workspaceBuildTargetsResult =
        if (!buildProject)
            query(true, "workspace/buildTargets") { server.workspaceBuildTargets() }!!.get()
        else
            query(true, "workspace/buildAndGetBuildTargets") {
                  server.workspaceBuildAndGetBuildTargets()
                }!!
                .get()
    ...
    val sourcesFuture =
        query(true, "buildTarget/sources") {
          server.buildTargetSources(SourcesParams(allTargetsIds))
        }!!

    // We have to check == true because bsp4j uses non-primitive Boolean (which is Boolean? in
    // Kotlin)
    val resourcesFuture =
        query(buildServerCapabilities.resourcesProvider == true, "buildTarget/resources") {
    ...
    val outputPathsFuture =
        query(buildServerCapabilities.outputPathsProvider == true, "buildTarget/outputPaths") {
...
): List<BuildTargetIdentifier> = workspaceBuildTargetsResult.targets.map { it.id }
...
private fun <T> CompletableFuture<T>.catchSyncErrors(
    errorCallback: (Throwable) -> Unit
): CompletableFuture<T> = this.whenComplete { _, exception -> exception?.let { errorCallback(it) } }

private fun OutputPathsResult.obtainDistinctUris(): List<String> =
    this.items.filterNotNull().flatMap { it.outputPaths }.map { it.uri }.distinct()
