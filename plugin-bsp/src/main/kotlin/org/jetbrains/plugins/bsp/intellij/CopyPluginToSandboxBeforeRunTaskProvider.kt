...
private val PROVIDER_ID =
    Key.create<CopyPluginToSandboxBeforeRunTaskProvider.Task>(
        "CopyPluginToSandboxBeforeRunTaskProvider")
...
public class CopyPluginToSandboxBeforeRunTaskProvider :
  ...
  override fun executeTask(
    ...
    for (pluginJar in pluginJars) {
      if (!pluginJar.exists()) {
        showError(
            BspPluginBundle.message("console.task.exception.plugin.jar.not.found", pluginJar),
            environment)
        return false
      }
      try {
  ...
  // Throwing an ExecutionException doesn't work from before run tasks, so we have to show the
  ...
  private fun getNotificationGroup(): NotificationGroup {
    return NotificationGroupManager.getInstance()
        .getNotificationGroup("CopyPluginToSandboxBeforeRunTaskProvider")
  }
}
