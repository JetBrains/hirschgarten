...
internal object DependencySourcesItemToLibraryTransformer :
  ...
  override fun transform(inputEntity: DependencySourcesAndJvmClassPaths): List<Library> {
    ...
    return librariesPreprocessedResult.libraries + toLibraryFromClassJars(unusedClassJars) +
      toLibraryFromSourceJars(
        unusedSourceJars,
      )
  }

  private fun preprocessLibraries(inputEntity: DependencySourcesAndJvmClassPaths): LibrariesPreprocessedResult {
  ...
  private fun toJarString(dependencyUri: String): String = Library.formatJarString(removeUriFilePrefix(dependencyUri))
