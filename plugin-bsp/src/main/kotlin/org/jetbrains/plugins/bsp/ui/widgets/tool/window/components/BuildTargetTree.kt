...
public class BuildTargetTree(
  ...
  private fun generateTreeFromIdentifiers(
      targets: List<BuildTargetTreeIdentifier>,
      separator: String?
  ) {
    val pathToIdentifierMap = targets.groupBy { it.path.firstOrNull() }
    val sortedDirs = pathToIdentifierMap.keys.filterNotNull().sorted()
    rootNode.removeAllChildren()
    for (dir in sortedDirs) {
      pathToIdentifierMap[dir]?.let { rootNode.add(generateDirectoryNode(it, dir, separator, 0)) }
    }
    pathToIdentifierMap[null]?.forEach { rootNode.add(generateTargetNode(it)) }
  }
  ...
  private fun generateChildrenNodes(
    ...
    val childrenTargetNodes = generateChildrenTargetNodes(pathToIdentifierMap)
  ...
  private fun generateChildrenDirectoryNodes(
      ...
      pathToIdentifierMap.keys.filterNotNull().sorted().mapNotNull { dir ->
        pathToIdentifierMap[dir]?.let { generateDirectoryNode(it, dir, separator, depth + 1) }
      }

  private fun generateChildrenTargetNodes(
      pathToIdentifierMap: Map<String?, List<BuildTargetTreeIdentifier>>,
  ): List<TreeNode> = pathToIdentifierMap[null]?.map { generateTargetNode(it) } ?: emptyList()

  private fun generateTargetNode(identifier: BuildTargetTreeIdentifier): DefaultMutableTreeNode =
      DefaultMutableTreeNode(
          TargetNodeData(
              identifier.id, identifier.target, identifier.displayName, identifier.target != null))
  ...
  override fun getSelectedBuildTarget(): BuildTargetInfo? {
    ...
    return if (userObject is TargetNodeData) userObject.target else null
  }
  ...
  public fun createNewWithTargetsAndHighlighter(
    ...
    val new =
        BuildTargetTree(
            targetIcon, invalidTargetIcon, buildToolId, newTargets, emptyList(), labelHighlighter)
