...
internal class ModuleDetailsToJavaModuleTransformer(
    ...
    ModuleDetailsToModuleTransformer<JavaModule>(
        targetsMap, moduleNameProvider, libraryNameProvider) {
  ...
  private val resourcesItemToJavaResourceRootTransformer =
      ResourcesItemToJavaResourceRootTransformer()
      ...
      JavaModule(
          ...
          moduleLevelLibraries =
              if (inputEntity.libraryDependencies == null)
                  DependencySourcesItemToLibraryTransformer.transform(
          ...
          jvmBinaryJars =
              inputEntity.jvmBinaryJars.flatMap { it.jars }.map { it.safeCastToURI().toPath() },
  ...
  override fun toGenericModuleInfo(inputEntity: ModuleDetails): GenericModuleInfo {
    ...
    return bspModuleDetailsToModuleTransformer
        .transform(bspModuleDetails)
        .applyHACK(inputEntity, projectBasePath)
  }

  private fun GenericModuleInfo.applyHACK(
      inputEntity: ModuleDetails,
      projectBasePath: Path
  ): GenericModuleInfo {
    val dummyJavaModuleDependencies =
        calculateDummyJavaModuleNames(
                calculateDummyJavaSourceRoots(toJavaSourceRoots(inputEntity)), projectBasePath)
  ...
  private fun toScalaAddendum(inputEntity: ModuleDetails): ScalaAddendum? {
    ...
    return ScalaAddendum(scalaSdkName = version)
  }
  ...
  private fun toJavaAddendum(inputEntity: ModuleDetails): JavaAddendum? =
      extractJvmBuildTarget(inputEntity.target)?.javaVersion?.let {
        JavaAddendum(languageVersion = it)
      }
...
public fun String.androidJarToAndroidSdkName(): String =
    "android-sdk-" + StringUtils.md5Hash(this, 5)
