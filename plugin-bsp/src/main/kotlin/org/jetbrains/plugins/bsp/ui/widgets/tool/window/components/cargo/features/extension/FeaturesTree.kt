...
public class FeaturesTree(
  ...
  init {
    treeComponent.selectionModel.selectionMode =
  ...
  private fun generatePackageNode(
    ...
    if (packageFeatures.availableFeatures.isEmpty()) {
      return null
    }

    val node = DefaultMutableTreeNode()
    node.userObject = PackageNodeData(packageFeatures.packageId)

    val enabledFeaturesForPackage =
      FeaturesService.getInstance(project).value.getEnabledFeaturesForPackage(
        packageFeatures.packageId
      )
    val childrenFeatureNodesList =
      generateFeatureNodes(
        packageFeatures.availableFeatures,
        enabledFeaturesForPackage
      ).toMutableList()
  ...
  private fun generateFeatureNodes(
    ...
    for (feature in availableFeatures) {
      ...
      featureNode.userObject =
        FeatureNodeData(feature.key, feature.value, isEnabled)
  ...
  private fun getSelectedFeatureAndPackage(): Selection? {
    val selectedFeature =
      treeComponent.lastSelectedPathComponent as? DefaultMutableTreeNode
    val featureNode = selectedFeature?.userObject as? FeatureNodeData

    val packageNodeTreeReference =
      selectedFeature?.parent as? DefaultMutableTreeNode
  ...
  private fun updateUiByTogglingGivenFeatures(
    featuresToToggle: Set<String>,
    packageTreeNode: DefaultMutableTreeNode,
    enabling: Boolean,
  ) {
  ...
  private fun handleTogglingFeature(selection: Selection, project: Project) {
    ...
    val state =
      featuresServiceValue.enabled.getOrPut(selection.`package`.id) { mutableSetOf() }
    if (enablingFeature)
      state += featuresToToggle
    else
      state -= featuresToToggle

    updateUiByTogglingGivenFeatures(
      featuresToToggle,
      selection.packageNodeTreeReference,
      enablingFeature
    )
  ...
  private fun updateServerState(
    project: Project,
    packageId: String,
    enabledFeaturesSet: Set<String>,
  ) {
    BspCoroutineService.getInstance(project).start {
      UpdateFeaturesTask(project, packageId, enabledFeaturesSet).execute()
        ?.let { statusCode ->
...
private class FeaturesTreeCellRenderer : TreeCellRenderer {
  ...
  override fun getTreeCellRendererComponent(
    ...
    return when (val userObject =
      (value as? DefaultMutableTreeNode)?.userObject) {
...
private data class Selection(
  val feature: FeatureNodeData,
  val `package`: PackageNodeData,
  val packageNodeTreeReference: DefaultMutableTreeNode,
)
