...
public class FeaturesServiceValue(
  ...
  public fun toState(): FeaturesServiceValueState {
    return FeaturesServiceValueState(
      ...
      packageIdToFlattenedEnablingMap.map { it.key to it.value.toState() }
        .toMap(),
      packageIdToFlattenedDisablingMap.map { it.key to it.value.toState() }
        .toMap(),
    )
  }
  ...
  public fun getEnabledFeaturesForPackage(packageId: String): Set<String> {
    return enabled.getOrPut(packageId) {
      val defaultState =
        getFlattenedEnablingMap(packageId).getOrDefault("default", emptySet())
          .toMutableSet()
  ...
  public fun getPackageFeatures(): Set<PackageFeatures> =
    packageFeatures.toSet()

  public fun getFlattenedEnablingMap(packageId: String): FlattenedFeaturesDependenciesMap =
    packageIdToFlattenedEnablingMap[packageId]
      ?: FlattenedFeaturesDependenciesMap(emptyMap())

  public fun getFlattenedDisablingMap(packageId: String): FlattenedFeaturesDependenciesMap =
    packageIdToFlattenedDisablingMap[packageId]
      ?: FlattenedFeaturesDependenciesMap(emptyMap())
}
...
public fun PackageFeatures.toState(): PackageFeaturesState =
  PackageFeaturesState(
    this.packageId,
