...
public abstract class BspServerTask<T>(
    private val taskName: String,
    protected val project: Project
) {
  protected fun connectAndExecuteWithServer(
      task: (JoinedBuildServer, BazelBuildServerCapabilities) -> T?
  ): T? = project.connection.runWithServer(task)
}
...
public abstract class BspServerSingleTargetTask<T>(taskName: String, project: Project) :
  ...
  public fun connectAndExecute(targetId: BuildTargetIdentifier): T? =
      connectAndExecuteWithServer { server, capabilities ->
        executeWithServer(server, capabilities, targetId)
      }
...
public abstract class BspServerMultipleTargetsTask<T>(taskName: String, project: Project) :
  ...
  public fun connectAndExecute(targetsIds: List<BuildTargetIdentifier>): T? =
      connectAndExecuteWithServer { server, capabilities ->
        executeWithServer(server, capabilities, targetsIds)
      }
