...
internal class ContentRootEntityUpdater(
  ...
  private fun addContentRootEntities(
    ...
    val contentRootEntities =
        entitiesToAdd.map { entityToAdd -> createContentRootEntity(moduleEntity, entityToAdd) }

    val updatedModuleEntity =
        builder.modifyModuleEntity(moduleEntity) { contentRoots += contentRootEntities }
  ...
  private fun createContentRootEntity(
    ...
    val url =
        entityToAdd.path.toVirtualFileUrl(workspaceModelEntityUpdaterConfig.virtualFileUrlManager)
    val excludedUrls =
        entityToAdd.excludedPaths.map {
  ...
  override fun addEntity(
      entityToAdd: ContentRoot,
      parentModuleEntity: ModuleEntity
  ): ContentRootEntity = addEntities(listOf(entityToAdd), parentModuleEntity).single()
}
