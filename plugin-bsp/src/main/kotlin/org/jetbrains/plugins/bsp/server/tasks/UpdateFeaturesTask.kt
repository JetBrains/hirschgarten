...
public class UpdateFeaturesTask(
  project: Project,
  private val packageId: String,
  private val featuresState: Set<String>,
) : BspServerTask<SetCargoFeaturesResult>("update features", project) {
  public fun execute(): StatusCode? {
    return try {
      val result = connectAndExecuteWithServer { server, _ ->
        server.setCargoFeatures(
          SetCargoFeaturesParams(
            packageId,
            featuresState.toList()
          )
        ).get()
      }
      result?.statusCode
