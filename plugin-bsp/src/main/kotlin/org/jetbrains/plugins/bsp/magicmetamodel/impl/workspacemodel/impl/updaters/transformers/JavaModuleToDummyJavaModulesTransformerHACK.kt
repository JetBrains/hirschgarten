...
/**
 * This is a HACK for letting single source Java files to be resolved normally Should remove soon
 * and replace with a more robust solution
 */
public class JavaModuleToDummyJavaModulesTransformerHACK(private val projectBasePath: Path) :
  ...
  override fun transform(inputEntity: JavaModule): List<JavaModule> {
    ...
    val dummyJavaModuleNames =
        calculateDummyJavaModuleNames(dummyJavaModuleSourceRoots, projectBasePath)
    return dummyJavaModuleSourceRoots.zip(dummyJavaModuleNames).mapNotNull {
...
internal fun calculateDummyJavaSourceRoots(sourceRoots: List<JavaSourceRoot>): List<Path> =
    sourceRoots.mapNotNull { restoreSourceRootFromPackagePrefix(it) }.distinct()
...
internal fun calculateDummyJavaModuleName(sourceRoot: Path, projectBasePath: Path): String? {
  ...
  // Don't create dummy Java modules for source roots outside the project directory so that they
  // aren't indexed
