...
public interface BuildTargetClassifierExtension : WithBuildToolId {
   ...
   * @return list of directories corresponding to a desired path of the given build target in the
   *   tree. For example, path `["aaa", "bbb"]` will render as:
   * ```
   * aaa
   *   | bbb
   *       | <buildTarget>
  ...
  public companion object {
    internal val ep =
        ExtensionPointName.create<BuildTargetClassifierExtension>(
            "org.jetbrains.bsp.buildTargetClassifierExtension")
  }
}

/**
 * Default implementation of the [BuildTargetClassifierExtension] interface. It will be used in BSP
 * project and when no other implementation exists.
 */
