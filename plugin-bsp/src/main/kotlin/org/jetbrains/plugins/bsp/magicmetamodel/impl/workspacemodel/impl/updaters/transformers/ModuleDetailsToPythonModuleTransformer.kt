...
internal class ModuleDetailsToPythonModuleTransformer(
    ...
    ModuleDetailsToModuleTransformer<PythonModule>(
  ...
  override fun transform(inputEntity: ModuleDetails): PythonModule =
      PythonModule(
          ...
          resourceRoots =
              resourcesItemToPythonResourceRootTransformer.transform(inputEntity.resources),
          libraries =
              DependencySourcesItemToPythonLibraryTransformer.transform(
                  inputEntity.dependenciesSources),
  ...
  private fun toSdkInfo(inputEntity: ModuleDetails): PythonSdkInfo? {
    ...
    return if (pythonBuildTarget != null &&
        pythonBuildTarget.version != null &&
        pythonBuildTarget.interpreter != null)
        PythonSdkInfo(
            version = pythonBuildTarget.version,
            originalName = inputEntity.target.id.uri,
        )
    else toDefaultSdkInfo(inputEntity, pythonBuildTarget)
  }

  private fun toDefaultSdkInfo(
