...
class ModuleDetailsToPythonModuleTransformerTest {
  ...
  fun `should return single python module for single module details`() {
    ...
    val expectedPythonModule =
        PythonModule(
            ...
            sourceRoots =
                listOf(
                    expectedGenericSourceRoot1,
                    expectedGenericSourceRoot2,
                    expectedGenericSourceRoot3),
  ...
  fun `should return multiple python modules for multiple module details`() {
    ...
    val targetsMap =
        listOf(buildTargetId1.uri, buildTargetId2.uri, buildTargetId3.uri).toDefaultTargetsMap()
    ...
    val expectedPythonSdk1 =
        PythonSdkInfo(
            version = defaultPythonVersion, originalName = "${buildTargetId1.uri}-detected")
    ...
    val expectedPythonModule1 =
        PythonModule(
            ...
            sourceRoots =
                listOf(
                    expectedGenericSourceRoot11,
                    expectedGenericSourceRoot12,
                    expectedGenericSourceRoot13),
    ...
    val expectedPythonSdk2 =
        PythonSdkInfo(
            version = defaultPythonVersion, originalName = "${buildTargetId2.uri}-detected")
  ...
  fun `should set default sdk info if there's no sdk provided`() {
    ...
    val expectedSdkInfo2 =
        PythonSdkInfo(version = defaultPythonVersion, "${buildTarget2.id.uri}-detected")
    val expectedSdkInfo3 =
        PythonSdkInfo(version = defaultPythonVersion, "${buildTarget3.id.uri}-detected")
