...
class JavaSourceEntityUpdaterTest : WorkspaceModelWithParentJavaModuleBaseTest() {
  ...
  fun `should add one java source root to the workspace model`() {
    ...
    loadedEntries(SourceRootEntity::class.java) shouldContainExactlyInAnyOrder
        listOf(expectedJavaSourceRootEntity)
  }
  ...
  fun `should add multiple java source roots to the workspace model`() {
    ...
    val expectedJavaSourceRootEntries =
        listOf(expectedJavaSourceRootEntity1, expectedJavaSourceRootEntity2)

    returnedJavaSourceRootEntries.map { it.sourceRoot } shouldContainExactlyInAnyOrder
        expectedJavaSourceRootEntries
    loadedEntries(SourceRootEntity::class.java) shouldContainExactlyInAnyOrder
        expectedJavaSourceRootEntries
  }
}
