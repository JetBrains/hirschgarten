...
class DependencySourcesItemToLibraryTransformerTest {
  ...
  fun `should return single library for dependency sources item with one dependency`() {
    ...
    // when
    val libraries =
        DependencySourcesItemToLibraryTransformer.transform(dependencySourcesAndJvmClasspaths)
    ...
    val expectedLibrary =
        Library(
            ...
            sourceJars =
                listOf("jar:///m2/repo.maven.apache.org/test/1.0.0/test-1.0.0-sources.jar!/"),
  ...
  fun `should return multiple libraries for dependency sources item with multiple dependencies and multiple (including non-overlapping classes)`() {
    ...
    val expectedLibrary3 =
        Library(
            displayName =
                "BSP: file:///m2/repo.maven.apache.org/test3/3.0.0/test3-3.0.0-sources.jar",
            sourceJars =
                listOf("jar:///m2/repo.maven.apache.org/test3/3.0.0/test3-3.0.0-sources.jar!/"),
        )
    libraries shouldContainExactlyInAnyOrder
        listOf(expectedLibrary1, expectedLibrary2, expectedLibrary3)
  }
  ...
  fun `should return multiple libraries for multiple dependency sources items`() {
    ...
    val dependencySourceItems =
        listOf(dependencySourcesAndJvmClassPaths1, dependencySourcesAndJvmClassPaths2)
