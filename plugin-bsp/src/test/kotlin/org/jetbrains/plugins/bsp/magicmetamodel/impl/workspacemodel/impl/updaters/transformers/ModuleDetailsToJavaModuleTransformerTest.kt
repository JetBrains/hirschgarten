...
class ModuleDetailsToJavaModuleTransformerTest {
  ...
  fun `should return no java modules roots for no modules details`() {
    ...
    val javaModules =
        ModuleDetailsToJavaModuleTransformer(
                mapOf(), DefaultModuleNameProvider, DefaultLibraryNameProvider, projectBasePath)
  ...
  fun `should return single java module for single module details`() {
    ...
    val javaModule =
        ModuleDetailsToJavaModuleTransformer(
                targetsMap, DefaultModuleNameProvider, DefaultLibraryNameProvider, projectBasePath)
            ...
            modulesDependencies =
                listOf(
                    ...
                    IntermediateModuleDependency(
                        calculateDummyJavaModuleName(projectRoot, projectBasePath)!!),
                ),
            librariesDependencies =
                listOf(
                    IntermediateLibraryDependency(
                        "BSP: file:///m2/repo.maven.apache.org/test1/1.0.0/test1-1.0.0.jar"),
                    IntermediateLibraryDependency(
                        "BSP: file:///m2/repo.maven.apache.org/test2/2.0.0/test2-2.0.0.jar"),
                ),
        )
    ...
    val expectedLibrary1 =
        Library(
            ...
            sourceJars =
                listOf("jar:///m2/repo.maven.apache.org/test1/1.0.0/test1-1.0.0-sources.jar!/"),
    ...
    val expectedLibrary2 =
        Library(
            ...
            sourceJars =
                listOf("jar:///m2/repo.maven.apache.org/test2/2.0.0/test2-2.0.0-sources.jar!/"),
    ...
    val expectedJavaModule =
        JavaModule(
            ...
            sourceRoots =
                listOf(expectedJavaSourceRoot1, expectedJavaSourceRoot2, expectedJavaSourceRoot3),
  ...
  fun `should return multiple java modules for multiple module details`() {
    ...
    val resourceFilePath11 =
        createTempFile(projectBasePath.toAbsolutePath(), "resource", "File1.txt")
    resourceFilePath11.toFile().deleteOnExit()
    val resourceFilePath12 =
        createTempFile(projectBasePath.toAbsolutePath(), "resource", "File2.txt")
        ...
        GenericModuleInfo(
            ...
            modulesDependencies =
                listOf(
                    ...
                    IntermediateModuleDependency("module3"),
                    IntermediateModuleDependency(
                        calculateDummyJavaModuleName(module1Root, projectBasePath)!!),
                ),
            librariesDependencies =
                listOf(
                    IntermediateLibraryDependency(
    ...
    val expectedJavaModule1 =
        JavaModule(
            ...
            sourceRoots =
                listOf(
                    expectedJavaSourceRoot11, expectedJavaSourceRoot12, expectedJavaSourceRoot13),
    ...
    javaModules shouldContainExactlyInAnyOrder
        (listOf(expectedJavaModule1, expectedJavaModule2) to
            { actual, expected ->
              validateJavaModule(actual, expected)
            })
  }
