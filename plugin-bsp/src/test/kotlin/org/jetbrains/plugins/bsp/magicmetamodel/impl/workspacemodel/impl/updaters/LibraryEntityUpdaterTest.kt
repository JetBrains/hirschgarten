...
internal class LibraryEntityUpdaterTest : WorkspaceModelBaseTest() {
  ...
  fun `should add one project library to the workspace model in place of module library`() {
    ...
    // when
    val returnedLibraryEntity = runTestWriteAction { libraryEntityUpdater.addEntity(library) }

    // then
    val expectedLibrarySourcesRoot =
        LibraryRoot(
            url =
                virtualFileUrlManager.getOrCreateFromUrl(
                    "jar:///dependency/test/1.0.0/test-1.0.0-sources.jar!/"),
            type = LibraryRootTypeId.SOURCES,
        )
    val expectedLibraryClassesRoot =
        LibraryRoot(
            url =
                virtualFileUrlManager.getOrCreateFromUrl(
                    "jar:///dependency/test/1.0.0/test-1.0.0.jar!/"),
            type = LibraryRootTypeId.COMPILED,
        )
    val expectedLibraryEntity =
    ...
    loadedEntries(LibraryEntity::class.java) shouldContainExactlyInAnyOrder
        listOf(expectedLibraryEntity)
  }
  ...
  fun `should add multiple project libraries to the workspace model in place of multiple module libraries`() {
    ...
    // when
    val returnedLibraryEntries = runTestWriteAction { libraryEntityUpdater.addEntries(libraries) }

    // then
    val expectedLibrarySourcesRoot1 =
        LibraryRoot(
            url =
                virtualFileUrlManager.getOrCreateFromUrl(
                    "jar:///dependency/test1/1.0.0/test1-1.0.0-sources.jar!/"),
            type = LibraryRootTypeId.SOURCES,
        )
    val expectedLibraryClassesRoot1 =
        LibraryRoot(
            url =
                virtualFileUrlManager.getOrCreateFromUrl(
                    "jar:///dependency/test1/1.0.0/test1-1.0.0.jar!/"),
            type = LibraryRootTypeId.COMPILED,
        )
    val expectedLibraryEntity1 =
        ExpectedLibraryEntity(
            libraryEntity =
                LibraryEntity(
                    tableId = LibraryTableId.ProjectLibraryTableId,
    ...
    val expectedLibrarySourcesRoot2 =
        LibraryRoot(
            url =
                virtualFileUrlManager.getOrCreateFromUrl(
                    "jar:///dependency/test2/2.0.0/test2-2.0.0-sources.jar!/"),
            type = LibraryRootTypeId.SOURCES,
        )
    val expectedLibraryClassesRoot2 =
        LibraryRoot(
            url =
                virtualFileUrlManager.getOrCreateFromUrl(
                    "jar:///dependency/test2/2.0.0/test2-2.0.0.jar!/"),
            type = LibraryRootTypeId.COMPILED,
        )
    val expectedLibraryEntity2 =
        ExpectedLibraryEntity(
            libraryEntity =
                LibraryEntity(
                    tableId = LibraryTableId.ProjectLibraryTableId,
