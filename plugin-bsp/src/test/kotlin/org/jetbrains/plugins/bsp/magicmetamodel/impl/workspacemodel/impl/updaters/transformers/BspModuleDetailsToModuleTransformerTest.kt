...
class BspModuleDetailsToModuleTransformerTest {
  ...
  fun `should return no modules for no bsp module details items`() {
    ...
    val modules =
        BspModuleDetailsToModuleTransformer(
                mapOf(), DefaultModuleNameProvider, DefaultLibraryNameProvider)
            .transform(emptyBspModuleDetails)
  ...
  fun `should return java module with dependencies to other targets and libraries`() {
    ...
    val module =
        BspModuleDetailsToModuleTransformer(
                targetsMap, DefaultModuleNameProvider, DefaultLibraryNameProvider)
        ...
        GenericModuleInfo(
            ...
            librariesDependencies =
                listOf(
                    ...
                    IntermediateLibraryDependency(
                        libraryName =
                            "BSP: file:///m2/repo.maven.apache.org/test2/1.0.0/test2-1.0.0.jar",
                        false,
                    ),
                ),
        )
  ...
  fun `should return module with associates when specified in BspModuleDetails`() {
    ...
    val targetsMap =
        listOf("//target1", "//target2", "//target3", "//target4", "//target5")
            .toDefaultTargetsMap()
  ...
  fun `should return multiple java modules with dependencies to other targets and libraries`() {
    ...
    modules shouldContainExactlyInAnyOrder
        (listOf(expectedModule1, expectedModule2) to
            { actual, expected ->
              shouldBeIgnoringDependenciesOrder(actual, expected)
            })
  }
  ...
  private fun shouldBeIgnoringDependenciesOrder(
      actual: GenericModuleInfo,
      expected: GenericModuleInfo
  ) {
