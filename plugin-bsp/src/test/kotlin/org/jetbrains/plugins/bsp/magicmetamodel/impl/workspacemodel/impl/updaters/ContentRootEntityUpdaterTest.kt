...
internal class ContentRootEntityUpdaterTest : WorkspaceModelWithParentJavaModuleBaseTest() {
  ...
  fun `should add one content root to the workspace model`() {
    ...
    // when
    runTestWriteAction { contentRootEntityUpdater.addEntity(contentRoot, parentModuleEntity) }
    ...
    val expectedContentRootEntity =
        ExpectedContentRootEntity(
            ...
            excludedUrls =
                listOf(
                    Path("/root/dir/example/resource/ExcludedFile.txt")
    ...
    loadedEntries(ContentRootEntity::class.java) shouldContainExactlyInAnyOrder
        listOf(expectedContentRootEntity)
  }
  ...
  fun `should add multiple content root to the workspace model`() {
    ...
    // when
    runTestWriteAction { contentRootEntityUpdater.addEntities(contentRoots, parentModuleEntity) }
