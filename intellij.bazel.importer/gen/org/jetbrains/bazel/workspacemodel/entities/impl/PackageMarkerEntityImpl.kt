...
internal class PackageMarkerEntityImpl(private val dataSource: PackageMarkerEntityData): PackageMarkerEntity, WorkspaceEntityBase(dataSource) {

private companion object {
internal val MODULE_CONNECTION_ID: ConnectionId = ConnectionId.create(ModuleEntity::class.java, PackageMarkerEntity::class.java, ConnectionId.ConnectionType.ONE_TO_MANY, false)
...
get() = snapshot.instrumentation.getParent(MODULE_CONNECTION_ID, this) as? ModuleEntity ?: error("Parent module not found for PackageMarkerEntity")
...
internal class Builder(result: PackageMarkerEntityData?): ModifiableWorkspaceEntityBase<PackageMarkerEntity, PackageMarkerEntityData>(result), PackageMarkerEntityBuilder {
...
((_diff as MutableEntityStorageInstrumentation).getParentBuilder(MODULE_CONNECTION_ID, this) as? ModuleEntityBuilder) ?: (this.entityLinks[EntityLink(false, MODULE_CONNECTION_ID)] as? ModuleEntityBuilder) ?: error("module is null for PackageMarkerEntity")
} else {
(this.entityLinks[EntityLink(false, MODULE_CONNECTION_ID)] as? ModuleEntityBuilder) ?: error("module is null for PackageMarkerEntity")
}
}
set(value){
checkModificationAllowed()
