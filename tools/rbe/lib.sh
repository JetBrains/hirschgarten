...
readonly RBE_BAZELRC_BEGIN_MARKER="# >>> plugins/bazel/tools/rbe/setup.sh >>>"
readonly RBE_BAZELRC_END_MARKER="# <<< plugins/bazel/tools/rbe/setup.sh <<<"
...
render_user_bazelrc() {
  cat <<EOF
$RBE_BAZELRC_BEGIN_MARKER
# Managed by plugins/bazel/tools/rbe/setup.sh.
# Remove this block to disable the local single-node remote execution worker.
build --config=rbe-local
test --config=rbe-local
run --config=rbe-local
coverage --config=rbe-local
$(render_rbe_profile)
$RBE_BAZELRC_END_MARKER
EOF
}
...
strip_managed_user_bazelrc_block() {
  local file="$1"
  local output_file="$2"
  ...
  start_count="$(grep -Fxc "$RBE_BAZELRC_BEGIN_MARKER" "$file" || true)"
  end_count="$(grep -Fxc "$RBE_BAZELRC_END_MARKER" "$file" || true)"

  [ "$start_count" -eq "$end_count" ] || die "Found mismatched local RBE bazelrc markers in $file."
  [ "$start_count" -le 1 ] || die "Found multiple local RBE bazelrc blocks in $file."

  awk -v start="$RBE_BAZELRC_BEGIN_MARKER" -v end="$RBE_BAZELRC_END_MARKER" '
...
trim_trailing_blank_lines() {
  ...
  temp_file="$(mktemp "${TMPDIR:-/tmp}/rbe-user-bazelrc.XXXXXX")"
  awk '
    { lines[NR] = $0 }
    END {
      last = NR
...
write_user_bazelrc_file() {
  local file="$1"
  local temp_file

  temp_file="$(mktemp "${TMPDIR:-/tmp}/rbe-user-bazelrc.XXXXXX")"
  strip_managed_user_bazelrc_block "$file" "$temp_file"
  trim_trailing_blank_lines "$temp_file"

  if [ -s "$temp_file" ]; then
    printf '\n' >> "$temp_file"
  fi

  render_user_bazelrc >> "$temp_file"
  mv "$temp_file" "$file"
}

write_user_bazelrcs() {
  write_user_bazelrc_file "$WORKSPACE_USER_BAZELRC"

  if [ -n "$PLUGIN_USER_BAZELRC" ]; then
    write_user_bazelrc_file "$PLUGIN_USER_BAZELRC"
  fi
}
