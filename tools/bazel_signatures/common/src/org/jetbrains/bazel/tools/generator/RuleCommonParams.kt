...
/**
 * Common, test-specific, and binary-specific parameters that Bazel's stardoc output
 * does not include in individual rule definitions. These are merged at generation time.
 *
 * Should not be needed after https://github.com/bazelbuild/stardoc/issues/292 is resolved.
 */
object RuleCommonParams {

  val common: List<ParamSignature> = listOf(
    ParamSignature(
      name = "aspect_hints",
      doc =
        """
  ...
  val test: List<ParamSignature> = listOf(
    ParamSignature(
      name = "args",
      required = false,
      named = true,
  ...
  val binary: List<ParamSignature> = listOf(
    ParamSignature(
      name = "args",
      required = false,
      positional = false,
  ...
  private fun List<ParamSignature>.withCleanedUpDocs() = map { it.copy(doc = it.doc?.cleanupStardoc()) }
}
