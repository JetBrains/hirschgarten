...
class TargetDiffService(
  ...
  suspend fun computeIncrementalDiff(contributor: TargetHashContributor, graph: BazelFastTargetGraph): SyncDiff {
  ...
  suspend fun computeIncrementalDiff(
    ...
    newHashes
      ...
      .forEach { (target, hash) -> target2Hash.put(target, hash.hash) }
  ...
  suspend fun computeFreshDiff(contributor: TargetHashContributor, graph: BazelFastTargetGraph): SyncDiff {
    val patterns = QueryTargetPattern.getProjectTargetUniverse(project)
    val newHashes = contributor.computeHashes(project, patterns)

    target2Hash.clear()
    newHashes.forEach { target2Hash.put(it.target, it.hash) }
