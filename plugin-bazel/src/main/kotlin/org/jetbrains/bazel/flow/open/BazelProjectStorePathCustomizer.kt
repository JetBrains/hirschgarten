...
private class BazelProjectStorePathCustomizer : ProjectStorePathCustomizer {
  override fun getStoreDirectoryPath(projectRoot: Path): ProjectStoreDescriptor? {
    ...
    val dotIdea = runReadActionBlocking { historicalProjectBasePath.selectDotIdeaPath(projectRoot) }
  ...
  private fun Path.selectDotIdeaPath(file: Path): Path {
    val rootDir = this.refreshAndFindVirtualDirectory() ?: return this.resolve(".idea")
    val projectViewFile = ProjectViewFileUtils.getProjectViewFilePath(file, this)
