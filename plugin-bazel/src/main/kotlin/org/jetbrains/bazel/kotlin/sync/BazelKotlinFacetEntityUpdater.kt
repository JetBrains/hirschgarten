...
class BazelKotlinFacetEntityUpdater : KotlinFacetEntityUpdater {
  ...
  private fun calculateKotlinSettingsEntity(
  ...
  ) = KotlinSettingsEntity.Companion(
    name = KotlinFacetType.Companion.NAME,
    ...
    version = KotlinFacetSettings.Companion.CURRENT_VERSION,
    flushNeeded = true,
  ) {
    ...
    this.targetPlatform = compilerArguments?.jvmTarget?.let { jvmTargetString ->
      JvmTarget.Companion.fromString(jvmTargetString)?.let { jvmTarget ->
