...
internal class BazelAttachSourcesProvider : AttachSourcesProvider {
  private class BazelAttachSourcesAction(private val project: Project) :
    ...
    override fun perform(orderEntries: List<LibraryOrderEntry>): ActionCallback {
      ...
      if (modelsToCommit.isNotEmpty())
          WriteAction.run<Exception> { modelsToCommit.forEach { it.commit() } }
    ...
    private fun Library.obtainModelWithAddedSources(
      ...
      else modifiableModel.apply { sources.forEach { addSource(it, fileManager) } }
    }

    private fun Library.ModifiableModel.addSource(
        sourceUri: String,
        fileManager: VirtualFileUrlManager
    ) {
      ...
      val sourceRoots =
          LibrarySourceRootDetectorUtil.scanAndSelectDetectedJavaSourceRoots(
              null, arrayOf(candidate))
      sourceRoots.forEach { source -> addRoot(source.url, OrderRootType.SOURCES) }
    }
  }
  ...
  private fun Project.containsBazelSourcesForEntries(
      orderEntries: List<LibraryOrderEntry>
  ): Boolean {
    val mmmLibraries = getAllMMMLibraries(this)
    return orderEntries.any {
      it.library?.pickSourcesFromBazel(mmmLibraries)?.isNotEmpty() ?: false
    }
  }
...
private fun Library.pickSourcesFromBazel(mmmLibraries: List<MMMLibrary>) =
    mmmLibraries.filter { it.displayName == name }.flatMap { it.sourceJars }
