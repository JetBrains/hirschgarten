...
internal class StarlarkRunLineMarkerContributor : RunLineMarkerContributor() {
  ...
  private fun PsiElement.calculateMarkerInfo(): Info? =
      containingFile.virtualFile?.url?.let { url ->
        ...
        val realTargetId =
            project.temporaryTargetUtils.allTargetIds().firstOrNull { it.uri.endsWith(targetId) }
        val targetInfo =
            realTargetId?.let { project.temporaryTargetUtils.getBuildTargetInfoForId(it) }
