...
class BlazeXmlToTestEventsConverter(
    ...
    private fun processAllTestResults(testResults: BlazeTestResults) {
        ...
        if (parsedResults != null) {
            parsedResults.forEach(Consumer { parsedResults: ParsedTargetResults? ->
    ...
    private fun processParsedTestResults(parsedResults: ParsedTargetResults) {
        if (noUsefulOutput(parsedResults.results, parsedResults.outputFiles)) {
            val status: Optional<BlazeTestResult.TestStatus?> =
                parsedResults.results.stream().map<Any?>(BlazeTestResult::getTestStatus).findFirst()
            status.ifPresent(
        ...
        processor.onTestFailure(
            getTestFailedEvent(
                ...
                STATUS_EXPLANATIONS.getOrDefault(status, "No output found for test target.")
                        + " See console output for details",  /* content= */
                null,
    ...
    companion object {
        ...
        private fun parseTestXml(
            label: Label, results: MutableCollection<BlazeTestResult?>
        ): ParsedTargetResults {
        ...
        private fun noUsefulOutput(
            results: MutableCollection<BlazeTestResult?>, outputFiles: MutableList<BlazeArtifact>
        ): Boolean {
            if (outputFiles.isEmpty()) {
                return true
            }
            val status: BlazeTestResult.TestStatus? =
                results.stream().map<Any?>(BlazeTestResult::getTestStatus).findFirst().orElse(null)
        ...
        private val STATUS_EXPLANATIONS: ImmutableMap<BlazeTestResult.TestStatus?, String?> =
            ImmutableMap.Builder<BlazeTestResult.TestStatus?, String?>()
        ...
        private fun isCancelled(test: BlazeXmlSchema.TestCase): Boolean {
            return "interrupted".equals(test.result, ignoreCase = true) || "cancelled".equals(
                test.result,
                ignoreCase = true
            )
        }
        ...
        private fun isFailed(test: BlazeXmlSchema.TestCase): Boolean {
            ...
            if (isIgnored(test)) {
                ...
                val errors: MutableList<ErrorOrFailureOrSkipped> =
                    if (!test.failures.isEmpty())
                        ...
                        if (!test.errors.isEmpty()) test.errors else ImmutableList.of<ErrorOrFailureOrSkipped?>(NO_ERROR)
        ...
        private fun getTestFailedEvent(
            name: String?, error: ErrorOrFailureOrSkipped, duration: Long, isError: Boolean
        ): TestFailedEvent {
            val message =
         ...
         */
        private fun bazelIsAtLeastVersion(major: Int, minor: Int, bugfix: Int): Boolean {
