...
abstract class TestContext internal constructor(
    ...
    internal class KnownTargetTestContext(
        ...
        override fun matchesTarget(config: BlazeCommandRunConfiguration): Boolean {
            return target.label.equals(config.singleTarget)
        }
    }
    ...
    companion object {
        fun builder(
            sourceElement: PsiElement?, supportedExecutors: ImmutableSet<ExecutorType?>?
        ): Builder {
            return TestContext.Builder(sourceElement, supportedExecutors)
        }
    }
}
