...
class BlazeConfigurationNameBuilder {
  ...
  constructor(configuration: BlazeCommandRunConfiguration) {
    ...
    if (!targets.isEmpty()) {
      val first: Label = targets[0]
      val text = getTextForLabel(first)
  ...
  fun setTargetString(label: Label): BlazeConfigurationNameBuilder {
    this.targetString =
      String.format("%s:%s", getImmediatePackage(label), label.targetName)
    return this
  }
  ...
  companion object {
     ...
     */
    fun getTextForLabel(label: Label): kotlin.String? = String.format("%s:%s", getImmediatePackage(label), label.targetName().toString())
