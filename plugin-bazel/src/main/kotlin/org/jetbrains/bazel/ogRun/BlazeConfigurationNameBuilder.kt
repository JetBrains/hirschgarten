...
class BlazeConfigurationNameBuilder {
    private var buildSystemName: String? = null
    ...
    constructor(configuration: BlazeCommandRunConfiguration) {
        ...
        val commandName: BlazeCommandName? = configuration.getHandler().commandName
        setCommandName(if (commandName == null) "command" else commandName.toString())

        val targets: ImmutableList<out TargetExpression> = configuration.targets
        if (!targets.isEmpty()) {
            val first: TargetExpression = targets.get(0)
            val text = if (first is Label) getTextForLabel(first as Label?) else first.toString()
            setTargetString(text)
        }
    }

    /**
     * Sets the build system name to the name of the build system used by `project`, e.g.
     * "Blaze" or "Bazel".
     */
    @com.google.errorprone.annotations.CanIgnoreReturnValue
    fun setBuildSystemName(project: Project?): BlazeConfigurationNameBuilder {
    ...
    companion object {
         ...
         */
        fun getTextForLabel(label: Label): kotlin.String? {
         ...
         */
        private fun getImmediatePackage(label: Label): kotlin.String {
