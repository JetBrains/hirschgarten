...
interface BlazeTestEventsHandler {
    ...
    fun suiteDisplayName(label: Label?, kind: Kind?, rawName: String?): String? {
        return rawName
    }

    /** Converts the testcase name in the blaze test XML to a user-friendly format.  */
    fun testDisplayName(label: Label?, kind: Kind?, rawName: String?): String? {
        return rawName
    }

    /** Converts the suite name to a parsable location URL.  */
    fun suiteLocationUrl(label: Label?, kind: Kind?, name: String?): String? {
    ...
    fun ignoreSuite(label: Label?, kind: Kind?, suite: BlazeXmlSchema.TestSuite): Boolean {
    ...
    companion object {
        ...
        fun targetsSupported(
            project: Project?, targets: ImmutableList<out TargetExpression?>
        ): Boolean {
            val kind: Kind? = getKindForTargets(project, targets)
            return Arrays.stream<BlazeTestEventsHandler?>(EP_NAME.extensions)
        ...
        fun getHandlerForTargetKindOrFallback(kind: Kind?): BlazeTestEventsHandler {
            return getHandlerForTargetKind(kind).orElse(BlazeGenericTestEventsHandler())
        }
        ...
        fun getHandlerForTarget(
            project: Project?, target: TargetExpression?
        ): Optional<BlazeTestEventsHandler> {
        ...
        fun getKindForTargets(project: Project?, targets: MutableList<out TargetExpression?>): Kind? {
        ...
        fun getKindForTarget(project: Project?, target: TargetExpression?): Kind? {
            if (target !is Label) {
                return null
            }
