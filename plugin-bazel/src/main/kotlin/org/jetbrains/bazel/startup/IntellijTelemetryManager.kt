...
object IntellijTelemetryManager : TelemetryManager {
  ...
  override fun getTracer(scope: Scope): Tracer = platformTelemetryManager.getTracer(IntellijScope(scope.name))

  override fun getMeter(scope: Scope): Meter = platformTelemetryManager.getMeter(IntellijScope(scope.name))
}
