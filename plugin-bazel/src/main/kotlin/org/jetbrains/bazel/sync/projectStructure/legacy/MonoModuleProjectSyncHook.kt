...
class MonoModuleProjectSyncHook : ProjectSyncHook {
  override fun isEnabled(project: Project): Boolean = EnableMonoModuleSyncHookExtension.EP.extensionList.any { it.isEnabled(project) }

  override suspend fun onSync(environment: ProjectSyncHook.ProjectSyncHookEnvironment) {
    val project = environment.project
    val moduleEntitySource = BazelModuleEntitySource(MONO_MODULE_NAME)
  ...
  interface EnableMonoModuleSyncHookExtension {
    fun isEnabled(project: Project): Boolean

    companion object {
      internal val EP = ExtensionPointName.create<EnableMonoModuleSyncHookExtension>("org.jetbrains.bazel.monoModuleSyncHook.enable")
    }
  }
}
