...
class BazelFileEventListener : BulkFileListenerBackgroundable {
  ...
  private suspend fun applyAllChanges(allFileEvents: List<SimplifiedFileEvent>, context: ProcessingContext) {
    ...
    val fileEventsToProcess = allFileEvents.filter { !it.isFileMoveInsideModule(existingModulesByEvent) }
  ...
  // BAZEL-2965 - if file is moved between two locations inside the same module, there's no need to process that event
  private fun SimplifiedFileEvent.isFileMoveInsideModule(
    ...
    for (module in modules) {
      ...
      val contentRootContainsBothFiles =
        contentRootPaths.any { fileAdded.startsWith(it) && fileRemoved.startsWith(it) }
  ...
  private suspend fun addNewFilesToBothModels(
    ...
    val bazelQueryIsRequired =
      addedFilePaths.any {
        (modulesAlreadyContainingFiles[it].orEmpty() - moduleToRemoveFilesFrom[it].orEmpty()).isEmpty()
      }
