...
public final class BlazeBeforeRunCommandHelper {
  ...
  public static ListenableFuture<BuildEventStreamProvider> runBlazeCommand(
      ...
      BlazeInvocationContext invocationContext,
      String progressMessage,
      ImmutableList<TargetPattern> targets) {
    ...
    String binaryPath =
        handlerState.getBlazeBinaryState().blazeBinary != null
            ? handlerState.getBlazeBinaryState().blazeBinary
  ...
  public static Path createScriptPathFile() throws IOException {
    ...
    if (Strings.isNullOrEmpty(dirPath)) {
      tempDir = FileOperationProvider.getInstance().getTempDirectory();
    } else {
