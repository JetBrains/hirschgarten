...
internal class BazelBuildTargetClassifier : BuildTargetClassifierExtension {
  ...
  override fun calculateBuildTargetName(buildTargetInfo: BuildTargetInfo): String =
      extractTargetIdComponents(buildTargetInfo.id.uri)?.targetName ?: buildTargetInfo.id.uri

  /**
   * This method uses string manipulation to extract useful components from Bazel target id, it
   * assumes the target id has the format `@?@?(?<repository>.*)//(?<package>.*):(?<target>.*)`
   */
