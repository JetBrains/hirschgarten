...
class StarlarkValue private constructor(
  ...
  override fun computeChildren(node: XCompositeNode) {
    computeYourChildrenPack?.let { pack ->
      pack.valueComputer
        .computeValueChildren(pack.threadId, pack.valueId)
        .thenAccept { node.addComputedChildren(it, pack) }
    } ?: node.addChildren(XValueChildrenList(), true)
  }

  private fun XCompositeNode.addComputedChildren(children: List<SDP.Value>, computeYourChildrenPack: ComputeYourChildrenPack) {
  ...
  companion object {
    fun fromProto(
      value: SDP.Value,
      threadId: Long,
      valueComputer: StarlarkValueComputer,
    ): StarlarkValue {
      val pack =
