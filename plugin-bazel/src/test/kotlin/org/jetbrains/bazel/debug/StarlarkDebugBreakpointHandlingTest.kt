...
class StarlarkDebugBreakpointHandlingTest : StarlarkDebugClientTestBase() {
  ...
  fun `breakpoint registration`() {
    ...
    conn.socket.readRequests().single().setBreakpoints.breakpointCount shouldBe 1
    conn.breakpointHandler.registerBreakpoint(breakpoint2)
    conn.socket.readRequests().single().setBreakpoints.breakpointCount shouldBe 2
    ...
    conn.socket.readRequests().single().setBreakpoints.breakpointCount shouldBe 0
  }
