...
class ResolverSanityTest : BasePlatformTestCase() {
  val testData = Runfiles.runfilesPath("/plugin-bazel/src/test/testData/sync/sanity").pathString
  ...
  fun testSampleRepoSanity() {
    val expectedOutput = getExpectedOutput("samplerepo")
    val actualOutput = getActualOutput("samplerepo")

    //Files.writeString(Path.of("/tmp/expected.txt"), expectedOutput)
    //Files.writeString(Path.of("/tmp/actual.txt"), actualOutput)

    assertEquals(expectedOutput, actualOutput)
  }

  fun testGoSanity() {
    val expectedOutput = getExpectedOutput("go")
    val actualOutput = getActualOutput("go")
  ...
  fun testPythonSanity() {
    val expectedOutput = getExpectedOutput("python")
    val actualOutput = getActualOutput("python")
  ...
  fun getExpectedOutput(name: String): String {
    return Path.of(testData).resolve("$name/expected_output.txt").toFile().readText()
  }

  fun getActualOutput(name: String): String {
    // Initialize providers for tests
    val targetInfoReader = TargetInfoReader(null)

    // Convert string arguments to Path objects
    val textprotoFiles = Files.walk(Path.of(testData).resolve(name))
      .filter { Files.isRegularFile(it) }
      .filter { it.extension == "textproto" }
      .toList()
      .toSortedSet()
    ...
    return runBlocking {
      ...
      return@runBlocking processWithUnifiedSetup(rawTargetsMap)
    }
  }

  private suspend fun processWithUnifiedSetup(rawTargetsMap: Map<Label, TargetInfo>): String {
    ...
    return processToPrettyString(rawTargetsMap, repoMapping, workspaceContext, featureFlags, bazelInfo)
  }

  private suspend fun processToPrettyString(
    ...
    val languagePluginsService = LanguagePluginsService(myFixture.project)
    ...
    return prettyPrint(resolvedWorkspace.toString())
  }
