  ...
  inner class DefaultProjectPreSyncHooks {
    ...
    fun beforeEach() {
      ...
      ProjectPreSyncHook.ep.registerExtension(
        _root_ide_package_.org.jetbrains.plugins.bsp.impl.flow.sync.TestProjectPreSyncHook(
          bspBuildToolId,
        ),
      )
    }
    ...
    fun `should return all enabled default project pre-sync hooks`() {
      // when & then
      ProjectPreSyncHook.ep.registerExtension(
        _root_ide_package_.org.jetbrains.plugins.bsp.impl.flow.sync.DisabledTestProjectPreSyncHook(
          bspBuildToolId,
        ),
      )

      project.defaultProjectPreSyncHooks.map { it::class.java } shouldContain
        _root_ide_package_.org.jetbrains.plugins.bsp.impl.flow.sync.TestProjectPreSyncHook::class.java
      project.defaultProjectPreSyncHooks.map { it::class.java } shouldNotContain
        _root_ide_package_.org.jetbrains.plugins.bsp.impl.flow.sync.DisabledTestProjectPreSyncHook::class.java
    }
  }
