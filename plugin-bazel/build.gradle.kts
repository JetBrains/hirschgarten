...
dependencies {
  ...
  // This is a fix for failing Intellij tests, needs to be adjusted to versions currently used by
  // the platform
...
subprojects {
  ...
  intellij { version.set(Platform.snapshotVersion) }

  tasks.withType(PublishPluginTask::class.java) { enabled = false }

  tasks.withType(VerifyPluginTask::class.java) { enabled = false }

  tasks.withType(BuildSearchableOptionsTask::class.java) { enabled = false }

  tasks.withType(RunIdeTask::class.java) { enabled = false }
}
...
intellij {
  ...
  pluginsRepositories { custom("https://plugins.jetbrains.com/plugins/nightly/20329") }
...
fun findLatestCompatibleBspPlugin(): String {
  ...
  val version =
      findLatestCompatiblePluginVersion(pluginId, buildVersion, "nightly")
...
tasks {
  ...
  prepareSandbox {
    ...
    runtimeClasspathFiles.set(runtimeConfiguration.exclude("org.jetbrains.bsp.protocol", null))
  }

  runPluginVerifier {
    ideVersions.set(
        pluginVerifierIdeVersions.split(',').map(String::trim).filter(String::isNotEmpty))
    failureLevel.set(
        setOf(
