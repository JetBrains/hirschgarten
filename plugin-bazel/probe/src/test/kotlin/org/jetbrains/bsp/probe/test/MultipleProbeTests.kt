...
/**
 * This is a sample to demonstrate how to create a suite that runs multiple tests on one installed
 * intellij instead of setting it up for every one separately.
 *
 * @Test fun `open project`() { with(runner) { runIntellij(runData) { val data = openProject(it,
 *   Option.apply(null)) val robot = data._2 ... Test logic goes here .. BoxedUnit.UNIT } } }
 */
class MultipleProbeTests {
  ...
  companion object {
    ...
    @JvmStatic
    fun setupIntellij() {
      runner = IdeProbeTestRunner("https://github.com/JetBrains/bazel-bsp.git", "3.1.0")
