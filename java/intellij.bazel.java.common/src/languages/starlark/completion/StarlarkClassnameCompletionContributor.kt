...
private class StarlarkClassnameCompletionProvider : CompletionProvider<CompletionParameters>() {
  companion object {
            ...
            .with(
              object : PatternCondition<StarlarkStringLiteralExpression>("isClassnameValue") {
                override fun accepts(element: StarlarkStringLiteralExpression, context: ProcessingContext?): Boolean =
                  StarlarkClassnameReferenceProvider.isClassnameValue(element)
              },
            ),
        )
    ...
    private const val KOTLIN_SYNTHETIC_CLASS = 3

    // KotlinClassHeader.Kind.MULTIFILE_CLASS_PART.id
    private const val KOTLIN_MULTIFILE_CLASS_PART = 5
  }
  ...
  private fun PsiClass.isCompletableClass(): Boolean {
    ...
    if (metadataKind == KOTLIN_SYNTHETIC_CLASS) return false

    // Exclude kotlin multi-file part implementations (e.g. FooKt__partN)
    if (metadataKind == KOTLIN_MULTIFILE_CLASS_PART) return false
