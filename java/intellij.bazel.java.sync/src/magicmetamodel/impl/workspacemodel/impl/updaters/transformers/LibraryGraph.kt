...
class LibraryGraph(private val libraries: List<LibraryItem>) {
  ...
  fun createLibraryModules(project: Project, defaultJdkName: String?): List<JavaModule> {
    return libraries
      .map { library ->
        ...
        val libraryModuleName = LibraryGraph.addLibraryModulePrefix(libraryName)
        JavaModule(
          genericModuleInfo =
            GenericModuleInfo(
  ...
  private fun DependencyLabel.toDependencyId(project: Project): String {
    ...
    return if (label.isLibraryId()) addLibraryModulePrefix(rawId) else rawId
  }

  companion object {
    @ApiStatus.Internal
    fun addLibraryModulePrefix(name: String): String = LIBRARY_MODULE_PREFIX + name
  }
}
