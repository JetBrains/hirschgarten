...
object BazelBspScalaProjectTest : BazelBspTestBaseScenario() {
  ...
  fun main(args: Array<String>) =
      try {
        ...
        testClient.client.logMessageNotifications.forEach { log.info(it.message) }
        throw t
      }

  override fun additionalServerInstallArguments() =
      arrayOf("-enabled-rules", "io_bazel_rules_scala", "rules_java", "rules_jvm")
  ...
  override fun expectedWorkspaceBuildTargetsResult(): WorkspaceBuildTargetsResult {
    ...
    return WorkspaceBuildTargetsResult(listOf(target))
  }
  ...
  private fun scalaOptionsResultsClasspathReceiver(): BazelBspTestScenarioStep {
    ...
    val scalaOptionsParams = ScalacOptionsParams(expectedTargetIdentifiers)
    return BazelBspTestScenarioStep("scalaOptions results") {
      testClientClasspathReceiver.testScalacOptions(
          60.seconds, scalaOptionsParams, expectedScalaOptionsResult)
    }
  }
  ...
  private fun compileWithWarnings(): BazelBspTestScenarioStep {
    ...
    return BazelBspTestScenarioStep("compile results") {
      testClient.testCompile(
          60.seconds, compileParams, expectedCompilerResult, listOf(expectedDiagnosticsParam))
