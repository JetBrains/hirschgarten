...
abstract class BazelBspAndroidProjectTestBase : BazelBspTestBaseScenario() {
  ...
  private fun compareWorkspaceBuildTargets(): BazelBspTestScenarioStep =
      BazelBspTestScenarioStep("Compare workspace/buildTargets") {
        testClient.test(timeout = 5.minutes) { session, _ ->
          ...
          testClient.assertJsonEquals<WorkspaceBuildTargetsResult>(
              expectedWorkspaceBuildTargetsResult(), result)
        }
      }

  private fun compareBuildTargetResources(): BazelBspTestScenarioStep =
      BazelBspTestScenarioStep("Compare buildTarget/resources") {
  ...
  private fun compareWorkspaceLibraries(): BazelBspTestScenarioStep =
      BazelBspTestScenarioStep("Compare workspace/libraries") {
        testClient.test(timeout = 5.minutes) { session, _ ->
          ...
          val appCompatLibrary =
              result.libraries.first { "androidx_appcompat_appcompat" in it.id.uri }
          ...
          val expectedJarNames =
              setOf("classes_and_libs_merged.jar", "AndroidManifest.xml", "res", "R.txt")
