...
object BazelBspRustProjectTest : BazelBspTestBaseScenario() {
  ...
  private fun workspaceBuildTargets(): BazelBspTestScenarioStep {
    ...
    return BazelBspTestScenarioStep("workspace build targets") {
      testClient.testWorkspaceTargets(1.minutes, workspaceBuildTargetsResult)
    }
  }

  override fun expectedWorkspaceBuildTargetsResult(): WorkspaceBuildTargetsResult =
      WorkspaceBuildTargetsResult(listOf(makeExampleLib(), makeExampleFeature(), makeExample()))

  private fun makeExampleLib(): BuildTarget {
    val exampleLibDependencies =
        listOf(BuildTargetIdentifier("@crate_index__instant-0.1.12//:instant"))
  ...
  private fun makeExampleFeature(): BuildTarget {
    ...
    return makeBuildTarget(
        "example", "example_foo", "application", exampleFeatureDependencies, true)
  }
  ...
  private fun rustWorkspaceResults(): BazelBspTestScenarioStep {
    val expectedTargetIdentifiers =
        expectedTargetIdentifiers().filter { it.uri != "bsp-workspace-root" }
    val expectedResolvedTargets =
        expectedTargetIdentifiers.filter { it.uri != "@//example:example" }
    ...
    return BazelBspTestScenarioStep("rustWorkspace results") {
      testClient.testRustWorkspace(30.seconds, rustWorkspaceParams, expectedRustWorkspaceResult)
    }
  }
  ...
  private fun expectedPackages(): List<RustPackage> {
    ...
    val exampleTargets =
        listOf(expectedExampleTarget("example"), expectedExampleTarget("example_foo", setOf("foo")))
    ...
    return listOf(
        ...
        expectedPackageFromDependency(
            "itertools", "0.10.5", setOf("default", "use_alloc", "use_std")),
        expectedPackageFromDependency("itoa", "1.0.6"),
        expectedPackageFromWorkspace("example-lib", exampleLibTargets, exampleLibTargets),
        expectedPackageFromWorkspace(
            "example", listOf(exampleTargets[1]), exampleTargets, setOf("foo")))
  }

  private fun expectedExampleTarget(name: String, features: Set<String> = setOf()): RustTarget =
      RustTarget(name, "file://\$WORKSPACE/example/main.rs", RustTargetKind.BIN, "2018", false)
          .also {
  ...
  private fun expectedDependencies(): Map<String, List<RustDependency>> {
    ...
    val exampleLibDependencies =
        createDependency(listOf(Pair("@crate_index__instant-0.1.12//", "instant")))
    val instantDependencies =
        createDependency(listOf(Pair("@crate_index__cfg-if-1.0.0//", "cfg_if")))
    val itertoolsDependencies =
        createDependency(listOf(Pair("@crate_index__either-1.8.1//", "either")))
  ...
  private fun createDependency(
      dependenciesNames: List<Pair<String, String>>
  ): List<RustDependency> =
  ...
  private fun expectedRawDependencies(): Map<String, List<RustRawDependency>> {
    val exampleDependencies =
        createRawDependency(
            listOf("@//example-lib:example_lib", "@crate_index__itertools-0.10.5//:itertools"))
    val exampleLibDependencies =
        createRawDependency(listOf("@crate_index__instant-0.1.12//:instant"))
