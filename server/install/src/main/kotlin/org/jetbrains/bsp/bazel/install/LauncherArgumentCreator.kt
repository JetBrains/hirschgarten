...
class LauncherArgumentCreator(private val installationContext: InstallationContext) {
  ...
  private fun readSystemProperty(name: String): String =
      System.getProperty(name)
          ?: throw NoSuchElementException("Could not read $name system property")
  ...
  fun debuggerConnectionArgv(): String? =
      installationContext.debuggerAddress?.value?.let {
        "-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=$it"
      }
