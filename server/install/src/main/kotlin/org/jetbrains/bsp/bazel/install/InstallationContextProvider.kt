...
object InstallationContextProvider {
  ...
  fun createInstallationContext(cliOptions: CliOptions): InstallationContext =
      InstallationContext(
          javaPath =
              cliOptions.javaPath?.let { InstallationContextJavaPathEntity(it) }
                  ?: InstallationContextJavaPathEntityMapper.default(),
          debuggerAddress =
              cliOptions.debuggerAddress?.let { InstallationContextDebuggerAddressEntity(it) },
  ...
  fun generateAndSaveProjectViewFileIfNeeded(cliOptions: CliOptions) {
    ...
    if (!generatedProjectViewFilePath.isFileExisted() || cliOptions.projectViewCliOptions != null) {
      if (!generatedProjectViewFilePath.exists()) {
        ProjectViewCLiOptionsProvider.generateProjectViewAndSave(
            cliOptions, generatedProjectViewFilePath)
      }
    }
  }

  private fun calculateGeneratedProjectViewPath(cliOptions: CliOptions): Path =
      cliOptions.projectViewFilePath
          ?: cliOptions.workspaceRootDir.resolve(DEFAULT_GENERATED_PROJECT_VIEW_FILE_NAME)
