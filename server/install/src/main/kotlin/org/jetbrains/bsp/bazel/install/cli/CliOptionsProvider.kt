...
class CliOptionsProvider(private val args: Array<String>) {
  ...
  init {
    val helpOption = Option.builder(HELP_SHORT_OPT).longOpt("help").desc("Show help.").build()
    cliParserOptions.addOption(helpOption)

    val workspaceRootDirectoryOption =
        Option.builder(WORKSPACE_ROOT_DIR_SHORT_OPT)
            .longOpt("directory")
            .hasArg()
            .argName("path")
            .desc(
    ...
    val bazelBinaryOption =
        Option.builder(BAZEL_BINARY_SHORT_OPT)
    ...
    val manualTargetsOption =
        Option.builder(BUILD_MANUAL_TARGETS_OPT)
    ...
    val directoriesOption =
        Option.builder(DIRECTORIES_SHORT_OPT)
            .longOpt("directories")
            .hasArgs()
            .argName("directories")
            .desc(
    ...
    val deriveTargetsFromDirectoriesOption =
        Option.builder(DERIVE_TARGETS_FLAG_SHORT_OPT)
    ...
    val importDepthOption =
        Option.builder(IMPORT_DEPTH_SHORT_OPT)
    ...
    val bazelWorkspaceRootDirOption =
        Option.builder(BAZEL_WORKSPACE_ROOT_DIR_OPT)
            .longOpt("bazel-workspace")
            .hasArg()
            .desc(
                "Add path to Bazel project's root directory. By default, it is the same as --directory (-d)")
            .build()
    cliParserOptions.addOption(bazelWorkspaceRootDirOption)

    val produceTraceLogOption =
        Option.builder(PRODUCE_TRACE_LOG_OPT)
            .longOpt("produce-trace-log")
            .desc("Server will create trace log file.")
            .build()
    cliParserOptions.addOption(produceTraceLogOption)

    val enabledRulesOption =
        Option.builder()
            .longOpt(ENABLED_RULES_LONG_OPT)
            .hasArgs()
            .desc("Add manually enabled rules to override the automatic rules detection mechanism")
            .build()
  ...
  private fun workspaceRootDir(cmd: CommandLine): Path =
      getOptionValueAndMapToAbsolutePath(cmd, WORKSPACE_ROOT_DIR_SHORT_OPT)
          ?: calculateCurrentAbsoluteDirectory()
  ...
  private fun javaPath(cmd: CommandLine): Path? =
      getOptionValueAndMapToAbsolutePath(cmd, JAVA_PATH_SHORT_OPT)

  private fun bazelBinary(cmd: CommandLine): Path? =
      getOptionValueAndMapToAbsolutePath(cmd, BAZEL_BINARY_SHORT_OPT)

  private fun getOptionValueAndMapToAbsolutePath(cmd: CommandLine, shortOpt: String): Path? =
      cmd.getOptionValue(shortOpt)
          ?.let(Paths::get)
          ?.let { calculateCurrentAbsoluteDirectory().resolve(it) }
          ?.let(Path::normalize)

  private fun debuggerAddress(cmd: CommandLine): String? =
      cmd.getOptionValue(DEBUGGER_ADDRESS_SHORT_OPT)

  private fun buildManualTargets(cmd: CommandLine): Boolean =
      cmd.hasOption(BUILD_MANUAL_TARGETS_OPT)

  private fun importDepth(cmd: CommandLine): Int? =
      cmd.getOptionValue(IMPORT_DEPTH_SHORT_OPT)?.toInt()

  private fun enabledRules(cmd: CommandLine): List<String>? =
      cmd.getOptionValues(ENABLED_RULES_LONG_OPT)?.toList()

  private fun ideJavaHomeOverride(cmd: CommandLine): Path? =
      getOptionValueAndMapToAbsolutePath(cmd, IDE_JAVA_HOME_OVERRIDE_LONG_OPT)

  private fun targets(cmd: CommandLine): List<String>? =
      cmd.getOptionValues(TARGETS_SHORT_OPT)?.toList()

  private fun excludedTargets(cmd: CommandLine): List<String>? =
      cmd.getOptionValues(EXCLUDED_TARGETS_LONG_OPT)?.toList()
  ...
  private fun calculateCurrentDir(): Path =
      System.getenv("BUILD_WORKING_DIRECTORY")?.let { Path(it) } ?: Path("")

  private fun directories(cmd: CommandLine): List<String>? =
      cmd.getOptionValues(DIRECTORIES_SHORT_OPT)?.toList()

  private fun excludedDirectories(cmd: CommandLine): List<String>? =
      cmd.getOptionValues(EXCLUDED_DIRECTORIES_LONG_OPT)?.toList()

  private fun deriveTargetsFlag(cmd: CommandLine): Boolean =
