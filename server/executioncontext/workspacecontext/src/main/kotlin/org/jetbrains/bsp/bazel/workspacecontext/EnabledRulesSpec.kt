...
internal object EnabledRulesSpecExtractor : ExecutionContextEntityExtractor<EnabledRulesSpec> {
  override fun fromProjectView(projectView: ProjectView): EnabledRulesSpec =
    EnabledRulesSpec(
      values =
        projectView.enabledRules?.values ?: emptyList(),
    )

  private fun mapNotEmptySection(enabledRulesSection: ProjectViewEnabledRulesSection): EnabledRulesSpec =
    EnabledRulesSpec(values = enabledRulesSection.values)
}
