...
internal object TargetsSpecExtractor : ExecutionContextEntityExtractor<TargetsSpec> {
  ...
  override fun fromProjectView(projectView: ProjectView): TargetsSpec =
      ...
      else deriveTargetsFromDirectoriesSectionFalse(projectView)
  ...
  private fun deriveTargetsFromDirectoriesSectionTrue(projectView: ProjectView): TargetsSpec =
      when {
        ...
        hasEmptyIncludedValuesAndNonEmptyExcludedValuesDirectories(projectView.directories!!) ->
  ...
  private fun deriveTargetsFromDirectoriesSectionFalse(projectView: ProjectView): TargetsSpec =
      when {
        ...
        hasEmptyIncludedValuesAndNonEmptyExcludedValues(projectView.targets!!) ->
            throw ExecutionContextEntityExtractorException(
  ...
  private fun hasEmptyIncludedValuesAndEmptyExcludedValues(
      targetsSection: ProjectViewTargetsSection
  ): Boolean = targetsSection.values.isEmpty() and targetsSection.excludedValues.isEmpty()

  private fun hasEmptyIncludedValuesAndNonEmptyExcludedValues(
      targetsSection: ProjectViewTargetsSection
  ): Boolean = targetsSection.values.isEmpty() and targetsSection.excludedValues.isNotEmpty()

  private fun hasEmptyIncludedValuesAndEmptyExcludedValuesDirectories(
      directoriesSection: ProjectViewDirectoriesSection
  ): Boolean = directoriesSection.values.isEmpty() and directoriesSection.excludedValues.isEmpty()

  private fun hasEmptyIncludedValuesAndNonEmptyExcludedValuesDirectories(
      directoriesSection: ProjectViewDirectoriesSection
  ): Boolean =
      directoriesSection.values.isEmpty() and directoriesSection.excludedValues.isNotEmpty()

  private fun mapNotEmptyNotDerivedTargetsSection(
      targetsSection: ProjectViewTargetsSection
  ): TargetsSpec = TargetsSpec(targetsSection.values, targetsSection.excludedValues)
  ...
  private fun mapNotEmptyDerivedTargetSection(
    ...
    val directoriesExcludedValues =
        directoriesSection.excludedValues.map { mapDirectoryToTarget(it) }
  ...
  private fun mapDirectoryToTarget(buildDirectoryIdentifier: Path): BuildTargetIdentifier =
      if (buildDirectoryIdentifier.pathString == ".") BuildTargetIdentifier("//...")
      else BuildTargetIdentifier("//" + buildDirectoryIdentifier.pathString + "/...")
}
