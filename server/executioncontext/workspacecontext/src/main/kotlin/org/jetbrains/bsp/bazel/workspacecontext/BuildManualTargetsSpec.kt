...
private val defaultBuildManualTargetsSpec =
  BuildManualTargetsSpec(
    value = false,
  )
...
internal object BuildManualTargetsSpecExtractor : ExecutionContextEntityExtractor<BuildManualTargetsSpec> {
  override fun fromProjectView(projectView: ProjectView): BuildManualTargetsSpec =
    if (projectView.buildManualTargets == null) {
      defaultBuildManualTargetsSpec
    } else {
      map(projectView.buildManualTargets!!)
    }
