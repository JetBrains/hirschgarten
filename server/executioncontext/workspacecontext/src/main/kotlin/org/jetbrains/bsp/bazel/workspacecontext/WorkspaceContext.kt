...
data class WorkspaceContext(
     ...
     *
     * Obtained from `ProjectView` simply by mapping 'targets' section or derived from 'directories'
     * if 'derive_targets_from_directories' is true.
     */
     ...
     *
     * Obtained from 'ProjectView' simply by mapping 'directories' section if not null, otherwise
     * the whole project is included (project root is included).
     */
...
class WorkspaceContextConstructor(workspaceRoot: Path) :
  ...
  override fun construct(projectView: ProjectView): WorkspaceContext {
    ...
    return WorkspaceContext(
        ...
        experimentalUseLibOverModSection =
            ExperimentalUseLibOverModSpecExtractor.fromProjectView(projectView),
        experimentalAddTransitiveCompileTimeJars =
            ExperimentalAddTransitiveCompileTimeJarsExtractor.fromProjectView(projectView),
    )
  }
}
