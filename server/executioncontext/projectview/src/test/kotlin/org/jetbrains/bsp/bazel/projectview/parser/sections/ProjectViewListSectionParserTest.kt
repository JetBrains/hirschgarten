...
class ProjectViewListSectionParserTest {
  ...
  companion object {
    @JvmStatic fun data(): List<Arguments> = listOf(buildFlagsSectionArguments())
    ...
    private fun buildFlagsSectionArguments(): Arguments {
      ...
      val sectionMapper = { values: List<String> -> ProjectViewBuildFlagsSection(values) }
    ...
    ): (List<String>) -> T = { includedElements: List<String> ->
      sectionMapper(mapElements(rawIncludedElementConstructor, elementMapper, includedElements))
    }
    ...
    ): List<V> = rawElements.map(rawIncludedElementConstructor).map(elementMapper)
  }
  ...
  internal inner class ParseRawSectionTest<V, T : ProjectViewListSection<V>> {
    ...
    fun shouldParseEmptySectionBody(
      ...
      // given
      val rawSection = ProjectViewRawSection(sectionName, "")
...
${rawElementConstructor("element2")}""")
      ...
      val rawSection4 =
          ProjectViewRawSection(sectionName, "\n\t" + rawElementConstructor("element3") + "\n\n\n")
      val rawSections =
