...
class ProjectViewExcludableListSectionParserTest<V, T : ProjectViewExcludableListSection<V>> {
  ...
  companion object {
    @JvmStatic fun data(): List<Arguments> = listOf(targetsSectionArguments())
    ...
    private fun targetsSectionArguments(): Arguments {
      ...
      val sectionConstructor =
          createSectionConstructor(
              { values: List<BuildTargetIdentifier>, excludedValues: List<BuildTargetIdentifier> ->
                ProjectViewTargetsSection(values, excludedValues)
              },
  ...
  internal inner class ParseRawSectionTest {
    ...
    fun shouldParseIncludedAndExcludedElements(
      ...
      // then
      val expectedSection =
          sectionConstructor(listOf("included1"), listOf("excluded1", "excluded2"))
      section shouldBe expectedSection
    }
  }
