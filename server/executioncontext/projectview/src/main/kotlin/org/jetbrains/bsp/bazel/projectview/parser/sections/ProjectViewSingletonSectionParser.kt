 ...
 *
 * @param <T> type of parsed single value section </T>
 */
abstract class ProjectViewSingletonSectionParser<V, T : ProjectViewSingletonSection<V>>
protected constructor(override val sectionName: String) : ProjectViewSectionParser<T>() {
  ...
  override fun parse(sectionBody: String): T? =
      sectionBody.trim().ifBlank { null }?.let { mapRawValue(it) }?.let { createInstance(it) }
...
object ProjectViewBazelBinarySectionParser :
    ProjectViewSingletonSectionParser<Path, ProjectViewBazelBinarySection>(
        ProjectViewBazelBinarySection.SECTION_NAME) {

  override fun mapRawValue(rawValue: String): Path = Path(rawValue)

  override fun createInstance(value: Path): ProjectViewBazelBinarySection =
      ProjectViewBazelBinarySection(value)
}
...
object ProjectViewBuildManualTargetsSectionParser :
    ProjectViewSingletonSectionParser<Boolean, ProjectViewBuildManualTargetsSection>(
        ProjectViewBuildManualTargetsSection.SECTION_NAME) {

  override fun mapRawValue(rawValue: String): Boolean = rawValue.toBoolean()

  override fun createInstance(value: Boolean): ProjectViewBuildManualTargetsSection =
      ProjectViewBuildManualTargetsSection(value)
}

object ProjectViewImportDepthSectionParser :
    ProjectViewSingletonSectionParser<Int, ProjectViewImportDepthSection>(
...
object ProjectViewIdeJavaHomeOverrideSectionParser :
    ProjectViewSingletonSectionParser<Path, ProjectViewIdeJavaHomeOverrideSection>(
        ProjectViewIdeJavaHomeOverrideSection.SECTION_NAME) {

  override fun mapRawValue(rawValue: String): Path = Path(rawValue)

  override fun createInstance(value: Path): ProjectViewIdeJavaHomeOverrideSection =
      ProjectViewIdeJavaHomeOverrideSection(value)
}

object ExperimentalUseLibOverModSectionParser :
    ProjectViewSingletonSectionParser<Boolean, ExperimentalUseLibOverModSection>(
        ExperimentalUseLibOverModSection.SECTION_NAME) {
