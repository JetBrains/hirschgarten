...
interface ProjectViewParser {
   ...
   * @return
   *
   * `ProjectView` if parsing has finished with success, it means:
   * 1) file under `projectViewFilePath` was successfully parsed (not all values have to be provided
   *    -- some fields in `ProjectView` might be ` Optional.empty`). <br></br>
   *
   * @throws Exception if:
   ...
   *
   * `ProjectView` if parsing has finished with success, it means:
   * 1) `projectViewFileContent` was successfully parsed (not all values have to be provided -- some
   *    fields in `ProjectView` might be null).
   *
