 ...
 */
abstract class ProjectViewListSectionParser<V, T : ProjectViewListSection<V>>
protected constructor(
  ...
  override fun parse(rawSections: ProjectViewRawSections): T? =
      try {
        parseAllSectionsAndMerge(rawSections)?.also {
          log.debug("Parsed '$sectionName' section. Result:\n$it")
        }
      } catch (e: Exception) {
...
object ProjectViewBuildFlagsSectionParser :
    ProjectViewListSectionParser<String, ProjectViewBuildFlagsSection>(
        ProjectViewBuildFlagsSection.SECTION_NAME) {

  override fun mapRawValues(rawValue: String): String = rawValue

  override fun createInstance(values: List<String>): ProjectViewBuildFlagsSection =
      ProjectViewBuildFlagsSection(values)
}

object ProjectViewEnabledRulesSectionParser :
    ProjectViewListSectionParser<String, ProjectViewEnabledRulesSection>(
        ProjectViewEnabledRulesSection.SECTION_NAME) {

  override fun mapRawValues(rawValue: String): String = rawValue

  override fun createInstance(values: List<String>): ProjectViewEnabledRulesSection =
