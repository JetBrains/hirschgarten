...
interface ProjectViewGenerator {
  ...
  /**
   * Saves provided project view (hopefuly using `generatePrettyStringRepresentation` result) in the
   * file (which could be parsed by the `ProjectViewParser`).
   *
  ...
  /**
   * Generates pretty (user-friendly) string representation of the provided project view which could
   * be saved in a file and then parsed with the `ProjectViewParser`.
   *
   * @param projectView - project view which pretty string should be generated
   * @return String with pretty representation of the project view file: <section 1 header>:
   *   <section 1 single value> <section 2 header>: <section 2 value 1> <section 2 value 2> <section
   *   2 value 3>
   * @see org.jetbrains.bsp.bazel.projectview.model.ProjectView
   * @see org.jetbrains.bsp.bazel.projectview.parser.ProjectViewParser
   */
