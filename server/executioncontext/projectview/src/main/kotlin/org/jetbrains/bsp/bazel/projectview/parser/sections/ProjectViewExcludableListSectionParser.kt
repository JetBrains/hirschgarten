 ...
 *
 * @param <T> type of parsed list section </T>
 */
abstract class ProjectViewExcludableListSectionParser<V, T : ProjectViewExcludableListSection<V>>
protected constructor(
  ...
  private fun filterExcludedEntries(entries: List<String>): List<String> =
      entries.filter { isExcluded(it) }.map { removeExcludedEntryPrefix(it) }
  ...
  private fun createInstanceOrEmpty(includedValues: List<V>, excludedValues: List<V>): T? =
      if (includedValues.isNotEmpty() or excludedValues.isNotEmpty())
          createInstance(includedValues, excludedValues)
      else null
...
object ProjectViewTargetsSectionParser :
    ProjectViewExcludableListSectionParser<BuildTargetIdentifier, ProjectViewTargetsSection>(
        ProjectViewTargetsSection.SECTION_NAME) {

  override fun mapRawValues(rawValue: String): BuildTargetIdentifier =
      BuildTargetIdentifier(rawValue)

  override fun createInstance(
      includedValues: List<BuildTargetIdentifier>,
      excludedValues: List<BuildTargetIdentifier>
...
object ProjectViewDirectoriesSectionParser :
    ProjectViewExcludableListSectionParser<Path, ProjectViewDirectoriesSection>(
        ProjectViewDirectoriesSection.SECTION_NAME) {

  override fun mapRawValues(rawValue: String): Path = Path(rawValue)

  override fun createInstance(
      includedValues: List<Path>,
      excludedValues: List<Path>
