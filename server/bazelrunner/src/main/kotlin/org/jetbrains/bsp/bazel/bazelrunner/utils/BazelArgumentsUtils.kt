...
object BazelArgumentsUtils {
  ...
  fun calculateExcludedTargetsWithExcludedPrefix(
      targets: List<BuildTargetIdentifier>
  ): List<String> = toRawUris(targets).map { "-$it" }
  ...
  private fun getMnemonicsForTargets(targets: String, languageIds: List<String>) =
      languageIds.joinToString(FUNCTIONS_DELIMITER) {
        getMnemonicForLanguageAndTargets(it, targets)
      }
  ...
  fun getQueryKindForPatternsAndExpressionsWithException(
      parameters: List<BazelQueryKindParameters>,
      exception: String
  ): String {
