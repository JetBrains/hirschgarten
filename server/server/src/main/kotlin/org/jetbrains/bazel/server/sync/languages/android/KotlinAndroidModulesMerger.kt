 ...
 */
class KotlinAndroidModulesMerger(private val workspaceContextProvider: WorkspaceContextProvider) {
  fun mergeKotlinAndroidModules(modules: List<Module>): List<Module> =
    if (workspaceContextProvider.currentFeatureFlags().isAndroidSupportEnabled) doMergeKotlinAndroidModules(modules) else modules
  ...
  private data class MergedKotlinAndroidModule(val mergedModule: Module, val oldModulesToRemove: List<Module>)
}
