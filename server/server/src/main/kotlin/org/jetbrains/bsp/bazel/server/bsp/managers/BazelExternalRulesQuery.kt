  ...
  override fun fetchExternalRuleNames(cancelChecker: CancelChecker): List<String> =
      when {
        enabledRules.isNotEmpty() ->
            BazelEnabledRulesQueryImpl(enabledRules).fetchExternalRuleNames(cancelChecker)
        isBzlModEnabled ->
            BazelBzlModExternalRulesQueryImpl(bazelRunner).fetchExternalRuleNames(cancelChecker) +
                BazelWorkspaceExternalRulesQueryImpl(bazelRunner)
...
class BazelWorkspaceExternalRulesQueryImpl(private val bazelRunner: BazelRunner) :
    BazelExternalRulesQuery {
  override fun fetchExternalRuleNames(cancelChecker: CancelChecker): List<String> =
      bazelRunner
          .commandBuilder()
...
class BazelBzlModExternalRulesQueryImpl(private val bazelRunner: BazelRunner) :
    BazelExternalRulesQuery {
...
data class BzlmodDependency(val key: String) {
  /**
   * Extract dependency name from bzlmod dependency, where the raw format is
   * `<DEP_NAME>@<DEP_VERSION>`.
   *
   * There were some issues with (empty) bzlmod projects and android, so the automatic mechanism
   * ignores it. Use `enabled_rules` to enable `rules_android` instead.
   */
