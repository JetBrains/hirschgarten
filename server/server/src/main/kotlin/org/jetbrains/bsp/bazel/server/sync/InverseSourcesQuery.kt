...
object InverseSourcesQuery {
  ...
  private fun targetLabels(
    ...
    val consistentLabelsArg =
        listOfNotNull(if (bazelRelease.major >= 6) "--consistent_labels" else null) // #bazel5
   ...
   * Null is returned in case the file does not exist or does not belong to any target's sources
   * list
   */
  private fun fileLabel(
    ...
    val fileLabelResult =
        bazelRunner
            ...
            .executeBazelCommand(null)
            .waitAndGetResult(cancelChecker)
    return if (fileLabelResult.statusCode == StatusCode.OK &&
        fileLabelResult.stdoutLines.size == 1) {
