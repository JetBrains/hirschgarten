...
class BazelProjectMapper(
  ...
  fun createProject(
    ...
    val dependencyGraph = measure("Build dependency tree") { DependencyGraph(rootTargets, targets) }
    val targetsToImport =
    ...
    val annotationProcessorLibraries =
        measure("Create AP libraries") { annotationProcessorLibraries(targetsToImport) }
    val kotlinStdlibsMapper =
        measure("Create kotlin stdlibs") { calculateKotlinStdlibsMapper(targetsToImport) }
    ...
    val scalaLibrariesMapper =
        measure("Create scala libraries") { calculateScalaLibrariesMapper(targetsToImport) }
    ...
    val librariesFromDepsAndTargets =
        measure("Libraries from targets and deps") {
          createLibraries(targetsAsLibraries) +
              librariesFromDeps.values.flatten().distinct().associateBy { it.label }
        }
    ...
    val sourceToTarget =
        measure("Build reverse sources") { buildReverseSourceMapping(mergedModulesFromBazel) }
    ...
    val invalidTargets =
        measure("Save invalid target labels") {
          removeDotBazelBspTarget(allTargetNames) -
              targetsToImport.map { Label.parse(it.id) }.toSet()
        }
  ...
  private fun <K, V> concatenateMaps(vararg maps: Map<K, List<V>>): Map<K, List<V>> =
      maps
          .flatMap { it.keys }
          .distinct()
          .associateWith { key -> maps.flatMap { it[key].orEmpty() } }
    ...
    return targetsToImport
        ...
        .map { target ->
          Label.parse(target.id) to
              listOf(createLibrary(Label.parse(target.id + "_output_jars"), target))
        }
        .toMap()
  }

  private fun annotationProcessorLibraries(
      targetsToImport: Sequence<TargetInfo>
  ): Map<Label, List<Library>> {
    return targetsToImport
  ...
  private fun calculateKotlinStdlibsMapper(
    ...
    val projectLevelKotlinStdlibsLibrary =
        calculateProjectLevelKotlinStdlibsLibrary(targetsToImport)
    val kotlinTargetsIds =
        targetsToImport.filter { it.hasKotlinTargetInfo() }.map { Label.parse(it.id) }
  ...
  private fun calculateProjectLevelKotlinStdlibsLibrary(
      targetsToImport: Sequence<TargetInfo>
  ): Library? {
  ...
  private fun calculateProjectLevelKotlinStdlibsJars(
      targetsToImport: Sequence<TargetInfo>
  ): Set<URI> =
      targetsToImport
  ...
  private fun calculateKotlincPluginLibrariesMapper(
      targetsToImport: Sequence<TargetInfo>
  ): Map<Label, List<Library>> =
      targetsToImport
          .filter {
            it.hasKotlinTargetInfo() && it.kotlinTargetInfo.kotlincPluginInfosList.isNotEmpty()
          }
          .associate {
  ...
  private fun calculateScalaLibrariesMapper(
      targetsToImport: Sequence<TargetInfo>
  ): Map<Label, List<Library>> {
    ...
    return scalaTargets.associateWith {
      languagePluginsService.scalaLanguagePlugin.scalaSdks[it.value]
          ?.compilerJars
          ?.mapNotNull { projectLevelScalaSdkLibraries[it] }
          .orEmpty()
    }
  }
  ...
  private fun createAidlLibrary(target: TargetInfo, workspaceContext: WorkspaceContext): Library? {
    ...
    if (target.sourcesList.isEmpty()) {
      // Bazel doesn't create the AIDL jar if there's no sources, since it'd be the same as the
      // output jar
  ...
  /**
   * In some cases, the jar dependencies of a target might be injected by bazel or rules and not are
   * not available via `deps` field of a target. For this reason, we read JavaOutputInfo's jdeps
   * file and filter out jars that have not been included in the target's `deps` list.
   *
  ...
  private fun getAllOutputJarsFromTransitiveDeps(
      ...
      outputJarsFromTransitiveDepsCache.getOrPut(targetOrLibrary) {
        val jarsFromTargets =
            targetsToImport[targetOrLibrary]
                ?.let { getTargetOutputJarsSet(it) + getTargetInterfaceJarsSet(it) }
                .orEmpty()
        val jarsFromLibraries =
            librariesToImport[targetOrLibrary]
                ?.let { it.outputs + it.interfaceJars }
                .orEmpty()
                .map { Paths.get(it.path) }
        val outputJars = (jarsFromTargets + jarsFromLibraries).toMutableSet()

        val dependencies =
            targetsToImport[targetOrLibrary]?.dependenciesList.orEmpty().map {
              Label.parse(it.id)
            } +
      ...
      targetInfo.jvmTargetInfo.jdepsList
          .flatMap { jdeps ->
            ...
            if (path.toFile().exists()) {
              val dependencyList =
                  path.inputStream().use { Deps.Dependencies.parseFrom(it).dependencyList }
              dependencyList
                  .asSequence()
  ...
  /**
   * Similar to what was done in the Google's Bazel plugin in JdepsFileReader#relevantDep, we should
   * only include deps that are actually used by the compiler
   */
  ...
  private fun targetSupportsJdeps(targetInfo: TargetInfo): Boolean {
    ...
    return setOf(Language.JAVA, Language.KOTLIN, Language.SCALA, Language.ANDROID)
  ...
  private fun syntheticLabel(lib: String): Label {
    ...
    return Label.parse(
        Paths.get(lib).fileName.toString().replace(replacementRegex, "-") + "-" + shaOfPath)
  }

  private fun createLibraries(targets: Map<Label, TargetInfo>): Map<Label, Library> {
    return targets
        .mapValues { (targetId, targetInfo) -> createLibrary(targetId, targetInfo) }
        .filterValues {
          it.interfaceJars.isNotEmpty() || it.sources.isNotEmpty() || it.outputs.isNotEmpty()
        }
  }
  ...
  private fun createLibrary(label: Label, targetInfo: TargetInfo): Library {
    ...
    return Library(
        label = label,
        outputs =
            getTargetOutputJarUris(targetInfo) + getAndroidAarUris(targetInfo) + intellijPluginJars,
        ...
        // Even when those JARs don't exist yet because they aren't built yet, we still need to pass
        // them.
  ...
  private fun createLibrariesFromTransitiveCompileTimeJars(
      ...
      if (workspaceContext.experimentalAddTransitiveCompileTimeJars.value) {
        val explicitCompileTimeInterfaces =
            calculateExplicitCompileTimeInterfaces(targetsToImport, targetsMap)
        val res = HashMap<Label, Library>()
        targetsToImport.associate { targetInfo ->
          val interfacesAndBinariesFromTarget =
              interfacesAndClassesFromTargetsToImport.getOrDefault(
                  Label.parse(targetInfo.id), emptySet())
  ...
  private fun getTargetOutputJarUris(targetInfo: TargetInfo) =
      getTargetOutputJarsList(targetInfo).map { it.toUri() }.toSet()
  ...
  private fun getAndroidAarUris(targetInfo: TargetInfo): Set<URI> {
    ...
    if (androidAarImportInfo.hasResourceFolder()) {
      result +=
          bazelPathsResolver.resolve(androidAarImportInfo.resourceFolder).resolve("res").toUri()
    }
  ...
  private fun getSourceJarUris(targetInfo: TargetInfo) =
      targetInfo.jvmTargetInfo.jarsList.flatMap { it.sourceJarsList }.resolveUris()
  ...
  private fun selectRustExternalTargetsToImport(
      ...
      graph.allTargetsAtDepth(-1, rootTargets).asSequence().filter {
  ...
  private fun selectTargetsToImport(
      ...
      graph
          .allTargetsAtDepth(workspaceContext.importDepth.value, rootTargets)
  ...
  private fun createModules(
      ...
      runBlocking(Dispatchers.Default) {
        targetsToImport
            ...
            .map {
              async {
                createModule(it, dependencyGraph, generatedLibraries[Label.parse(it.id)].orEmpty())
              }
            }
            .awaitAll()
  ...
  private fun isLanguageFile(file: FileLocation, language: Language): Boolean =
      language.extensions.any { file.relativePath.endsWith(it) }
  ...
  private fun Label.toDirectoryUri(): URI {
    ...
    val path =
        if (isWorkspace) bazelPathsResolver.extractRelativePath(value)
        else bazelPathsResolver.extractExternalPath(value)
  ...
  private fun resolveResources(target: TargetInfo, languagePlugin: LanguagePlugin<*>): Set<URI> =
      bazelPathsResolver.resolveUris(target.resourcesList).toSet() +
          languagePlugin.resolveAdditionalResources(target)
  ...
  private fun buildReverseSourceMappingForModule(module: Module): List<Pair<URI, Label>> =
      with(module) { (sourceSet.sources + resources).map { Pair(it, label) } }
  ...
  private fun removeDotBazelBspTarget(targets: List<Label>): List<Label> {
    return targets.filter {
      bazelInfo.release.isRelativeWorkspacePath(it.value) &&
          !bazelInfo.release.stripPrefix(it.value).startsWith(".bazelbsp")
    }
  }
  ...
  private fun createRustExternalModules(
    ...
    return modules.asSequence().onEach { (it.languageData as? RustModule)?.isExternalModule = true }
  }
}
