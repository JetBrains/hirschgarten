...
class JdkResolver(
  ...
  fun resolve(targets: Sequence<TargetInfo>): Jdk? {
    val allCandidates =
        targets.mapNotNull { resolveJdkData(it) }.sortByFrequency().map { JdkCandidate(it) }
    ...
    return (pickCandidateFromJvmRuntime(latestVersionAndComplete)
  ...
  private fun candidatesWithLatestVersion(
      candidates: Sequence<JdkCandidate>
  ): Sequence<JdkCandidate> =
  ...
  private fun resolveJdkData(targetInfo: TargetInfo): JdkCandidateData? {
    val hasRuntimeJavaHome =
        targetInfo.hasJavaRuntimeInfo() && targetInfo.javaRuntimeInfo.hasJavaHome()
    val hasToolchainJavaHome =
        targetInfo.hasJavaToolchainInfo() && targetInfo.javaToolchainInfo.hasJavaHome()

    val javaHomeFile =
        if (hasRuntimeJavaHome) targetInfo.javaRuntimeInfo.javaHome
        else if (hasToolchainJavaHome) targetInfo.javaToolchainInfo.javaHome else null
    val javaHome = javaHomeFile?.let { bazelPathsResolver.resolveUri(it) }

    return JdkCandidateData(hasRuntimeJavaHome, javaHome).takeIf { javaHome != null }
  }
  ...
  private fun <A> Sequence<A>.sortByFrequency(): Sequence<A> =
      groupBy { it }.values.sortedByDescending { it.size }.map { it.first() }.asSequence()
}
