...
object ClasspathQuery {
  fun classPathQuery(
    ...
    if (cqueryResult.isNotSuccess)
        throw RuntimeException("Could not query target '${target.uri}' for runtime classpath")
    try {
      val classpaths = Gson().fromJson(cqueryResult.stdout, JvmClasspath::class.java)
      return classpaths
    } catch (e: JsonSyntaxException) {
      // sometimes Bazel returns two values to a query when multiple configurations apply to a
      // target
