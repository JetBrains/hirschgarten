...
object BazelRootMessageParser : Parser {
  ...
  private fun findErrorInBUILD(output: Output): List<Diagnostic>? {
    return output.tryTake(ErrorInBUILD)?.let { match ->
      return collectCompilerDiagnostics(output).ifEmpty {
        listOf(createError(match, output.targetLabel))
      }
    }
  }
  ...
  private fun findWarningsInInfoMessage(output: Output): List<Diagnostic>? {
    return output.tryTake(InfoMessage)?.let { collectCompilerDiagnostics(output) }
  }

  private fun collectCompilerDiagnostics(output: Output) =
      generateSequence {
            CompilerDiagnosticParser.tryParseOne(output)
                ?: Scala3CompilerDiagnosticParser.tryParseOne(output)
          }
          .toList()
}
