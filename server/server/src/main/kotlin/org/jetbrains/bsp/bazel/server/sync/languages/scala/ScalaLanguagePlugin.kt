...
class ScalaLanguagePlugin(
  ...
  override fun prepareSync(targets: Sequence<BspTargetInfo.TargetInfo>) {
    scalaSdks =
        targets
            .associateBy({ it.id }, ScalaSdkResolver(bazelPathsResolver)::resolveSdk)
            .filterValuesNotNull()
  }
  ...
  fun toScalaTestClassesItem(module: Module): ScalaTestClassesItem? =
      if (!module.tags.contains(Tag.TEST) || !module.languages.contains(Language.SCALA)) null
      else
  ...
  fun toScalaMainClassesItem(module: Module): ScalaMainClassesItem? =
      if (!module.tags.contains(Tag.APPLICATION) || !module.languages.contains(Language.SCALA)) null
      else
