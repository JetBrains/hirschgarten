...
class ProjectResolver(
      ...
      tracer.spanBuilder("Resolve project").use {
        ...
        val bazelExternalRulesQuery =
            BazelExternalRulesQueryImpl(
                bazelRunner, bazelInfo.isBzlModEnabled, workspaceContext.enabledRules)

        val externalRuleNames =
            measured("Discovering supported external rules") {
              bazelExternalRulesQuery.fetchExternalRuleNames(cancelChecker)
            }

        val ruleLanguages =
            measured("Mapping rule names to languages") {
        ...
        val buildAspectResult =
            measured("Building project with aspect") {
              buildProjectWithAspect(
                  cancelChecker, workspaceContext, keepDefaultOutputGroups = build)
            }
        val aspectOutputs =
            measured("Reading aspect output paths") {
              buildAspectResult.bepOutput.filesByOutputGroupNameTransitive(BSP_INFO_OUTPUT_GROUP)
            }
        val targets =
            measured("Parsing aspect outputs") {
              targetInfoReader.readTargetMapFromAspectOutputs(aspectOutputs)
            }
        val allTargetNames =
            if (buildAspectResult.isFailure)
                measured("Fetching all possible target names") {
                  formatTargetsIfNeeded(
                      bazelBspFallbackAspectsManager.getAllPossibleTargets(cancelChecker), targets)
                }
            else emptyList()
        val rootTargets =
            buildAspectResult.bepOutput.rootTargets().let { formatTargetsIfNeeded(it, targets) }
        return measured("Mapping to internal model") {
          bazelProjectMapper.createProject(
              targets, rootTargets.toSet(), allTargetNames, workspaceContext, bazelInfo)
        }
      }
  ...
  private fun buildProjectWithAspect(
    ...
    val outputGroups =
        listOf(BSP_INFO_OUTPUT_GROUP, ARTIFACTS_OUTPUT_GROUP, RUST_ANALYZER_OUTPUT_GROUP).map {
          if (keepDefaultOutputGroups) "+$it" else it
        }
    ...
    return bazelBspAspectsManager.fetchFilesFromOutputGroups(
        ...
        shouldBuildManualFlags =
            workspaceContext.shouldAddBuildAffectingFlags(keepDefaultOutputGroups),
        isRustEnabled = workspaceContext.isRustEnabled,
    )
  }

  private fun formatTargetsIfNeeded(
      targets: Collection<Label>,
      targetsInfo: Map<Label, BspTargetInfo.TargetInfo>
  ): List<Label> =
      when (bazelInfo.release.major) {
        ...
        in 0..5 -> targets.toList()
            ...
            targets
                .map { if (targetsInfo.contains(Label.parse("@@${it.value}"))) "@@$it" else "@$it" }
                .map { Label.parse(it) }
      }.toList()
