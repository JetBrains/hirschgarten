 ...
 * - References from `foo_base` to `foo_kt` are not resolved - we would have to add a circular
 *   dependency;
 * - Test classes are inside `foo_kt`, so the BSP client calls buildTargetTest for `foo_kt` instead
 *   of `foo`;
 * - Three targets are displayed for the user instead of one that they defined in the BUILD file.
 *
 * Therefore, we just merge them all into a single target. Also see
 * [Android Kotlin rules implementation](https://github.com/bazelbuild/rules_kotlin/blob/a675511fdbee743c09d537c2dddfb349981ae70b/kotlin/internal/jvm/android.bzl).
 */
class KotlinAndroidModulesMerger {
  fun mergeKotlinAndroidModules(
      modules: List<Module>,
  ...
  private fun tryMergeKotlinAndroidModule(
    ...
    val androidLanguageData =
        if (androidModule.resources.isNotEmpty()) {
    ...
    val kotlinAndroidLanguageData =
        androidLanguageData.copy(kotlinModule = kotlinLanguageData, javaModule = javaModule)

    val mergedDependencies =
        kotlinModule.directDependencies.asSequence() + androidModule.directDependencies.asSequence()
    val correctMergedDependencies =
        mergedDependencies
            .filterNot { it == androidModule.label }
            .filterNot { it == kotlinModule.label }
            .filterNot {
              it.value.endsWith("//third_party:android_sdk")
            } // This is added by Kotlin rules
            .distinct()
            .toList()
    ...
    val mergedModule =
        Module(
            ...
            environmentVariables =
                kotlinModule.environmentVariables + androidModule.environmentVariables,
        )
