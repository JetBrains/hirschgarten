...
class DebugRunner(
      ...
      when (debugType) {
        ...
        is DebugType.JDWP -> {
          if (!moduleToRun.isJavaOrKotlin()) {
            errorMessageSender(
                "JDWP debugging is only available for Java and Kotlin targets", originId)
            false
          } else true
        }
  ...
  // TODO [#BAZEL-721] - quite a naive predicate, but otherwise we'll need to have rule type info in
  // Module instance
  private fun Module.isJavaOrKotlin() =
      languages.contains(Language.JAVA) || languages.contains(Language.KOTLIN)
  ...
  private fun jdwpArgument(port: Int): String =
      // all used options are defined in
