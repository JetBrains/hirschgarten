...
class PythonLanguagePlugin(private val bazelPathsResolver: BazelPathsResolver) :
  ...
  private fun calculateDefaultTargetInfo(targets: Sequence<TargetInfo>): PythonTargetInfo? =
      targets.filter(::hasPythonInterpreter).firstOrNull()?.pythonTargetInfo
  ...
  override fun resolveModule(targetInfo: TargetInfo): PythonModule? =
      targetInfo.pythonTargetInfo?.let { pythonTargetInfo ->
        PythonModule(
            calculateInterpreterURI(interpreter = pythonTargetInfo.interpreter)
                ?: defaultInterpreter,
