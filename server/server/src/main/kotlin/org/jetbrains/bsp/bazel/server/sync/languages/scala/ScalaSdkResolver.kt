...
class ScalaSdkResolver(private val bazelPathsResolver: BazelPathsResolver) {
  ...
  fun resolveSdk(targetInfo: BspTargetInfo.TargetInfo): ScalaSdk? {
    ...
    val compilerJars = bazelPathsResolver.resolvePaths(scalaTarget.compilerClasspathList).sorted()
    ...
    return ScalaSdk(
        "org.scala-lang", version, binaryVersion, compilerJars.map(bazelPathsResolver::resolveUri))
  }
  ...
  companion object {
    private val VERSION_PATTERN =
        Pattern.compile(
            "(?:processed_)?scala3?-(?:library|compiler|reflect)(?:_3)?-([.\\d]+)\\.jar")
  }
}
