...
/**
 * Every Kotlin Android target actually produces three targets, which we merge inside
 * [KotlinAndroidModulesMerger]. However, in order for all the dependent libraries to be unpacked
 * properly (e.g. for Jetpack Compose preview), we still have to pass the dependent Kotlin target
 * explicitly during build (and not just the merged target).
 */
