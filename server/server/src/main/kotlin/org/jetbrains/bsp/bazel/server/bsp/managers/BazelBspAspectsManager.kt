...
class BazelBspAspectsManager(
  ...
  fun calculateRuleLanguages(externalRuleNames: List<String>): List<RuleLanguage> =
      Language.values().mapNotNull { language ->
        if (language.ruleNames.isEmpty())
            return@mapNotNull RuleLanguage(null, language) // bundled in Bazel
          ...
          val variableMap =
              mapOf(
                  ...
                  "addTransitiveCompileTimeJars" to
                      if (workspaceContext.experimentalAddTransitiveCompileTimeJars.value) "True"
                      else "False",
              )
  ...
  fun fetchFilesFromOutputGroups(
    ...
    if (targetSpecs.values.isEmpty())
        return BazelBspAspectsManagerResult(BepOutput(), isFailure = false)
    val defaultFlags =
        listOf(
    ...
    val buildManualTargetsFlags =
        if (shouldBuildManualFlags) listOf(buildManualTests()) else emptyList()
    ...
    return bazelBspCompilationManager
        .buildTargetsWithBep(
            ...
            // (`Cargo.toml` file) and syncs `lockfile` (`Cargo.bazel.lock` file) with
            // `cargo_lockfile`.
            ...
            environment =
                if (isRustEnabled) listOf(Pair("CARGO_BAZEL_REPIN", "1")) else emptyList(),
        )
        .let { BazelBspAspectsManagerResult(it.bepOutput, it.processResult.isNotSuccess) }
  }
}
