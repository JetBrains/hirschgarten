...
class BspRequestsRunner(private val serverLifetime: BazelBspServerLifetime) {
  ...
  private fun <T> runAsync(
      methodName: String,
      request: Function<CancelChecker, T>
  ): CompletableFuture<T> {
    ...
    return CancellableFuture.from(CompletableFutures.computeAsync(requestWrapped))
        ...
        .thenCompose { either: Either<Throwable, T> ->
          if (either.isLeft) failure(methodName, either.left)
  ...
  private fun <T> failure(methodName: String, throwable: Throwable): CompletableFuture<T> {
    ...
    return CompletableFuture.failedFuture(
        ResponseErrorException(ResponseError(ResponseErrorCode.InternalError, message, null)))
  }

  companion object {
    private val LOGGER = LogManager.getLogger(BspRequestsRunner::class.java)
  }
}
