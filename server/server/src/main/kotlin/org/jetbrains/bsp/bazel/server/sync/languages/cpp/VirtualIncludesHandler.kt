...
class VirtualIncludesHandler(val pathResolver: CppPathResolver) {
  ...
  private val EXTERNAL_DIRECTORY_IDX: Int = 3
  private val EXTERNAL_WORKSPACE_NAME_IDX: Int = 4
  private val WORKSPACE_PATH_START_FOR_EXTERNAL_WORKSPACE: Int = 5
  private val WORKSPACE_PATH_START_FOR_LOCAL_WORKSPACE: Int = 3
  ...
  fun resolveVirtualInclude(
    ...
    val externalRoot = Path.of("external")
  ...
  private fun guessTargetKey(path: Path): String? {
    ...
    val virtualIncludesIdx: Int = split.indexOf(VIRTUAL_INCLUDES_DIRECTORY)

    if (virtualIncludesIdx > -1) {
      val externalWorkspaceName: String? =
        if (split[EXTERNAL_DIRECTORY_IDX] == Path.of("external")) split[EXTERNAL_WORKSPACE_NAME_IDX].toString() else null

      val workspacePathStart: Int =
        if (externalWorkspaceName != null) WORKSPACE_PATH_START_FOR_EXTERNAL_WORKSPACE else WORKSPACE_PATH_START_FOR_LOCAL_WORKSPACE

      val workspacePaths: List<Path> = if (workspacePathStart <= virtualIncludesIdx)
        split.subList(workspacePathStart, virtualIncludesIdx)
      else emptyList()

      val workspacePathString: String =
        workspacePaths
          .reduce { obj: Path, other: Path? -> obj.resolve(other) }.toString()
  ...
  fun String.toSystemIndependentName(): String {
    return replace('\\', '/')
  }
  ...
  fun createLabelName(
    externalWorkspaceName: String?, packagePath: Path, targetName: String
  ): String {
    return "${if (externalWorkspaceName != null) "@$externalWorkspaceName" else ""}//$packagePath:$targetName"
