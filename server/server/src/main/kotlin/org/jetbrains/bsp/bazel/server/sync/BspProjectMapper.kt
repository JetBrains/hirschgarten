...
class BspProjectMapper(
        ...
        project.libraries.values.map {
          ...
          if (it.keepNonExistentJars) {
            ...
            LibraryItem(
                ...
                dependencies = dependencies,
                ijars =
                    it.interfaceJars.filter { o -> o.toPath().exists() }.map { o -> o.toString() },
                jars = it.outputs.filter { o -> o.toPath().exists() }.map { uri -> uri.toString() },
                sourceJars =
                    it.sources.filter { o -> o.toPath().exists() }.map { uri -> uri.toString() },
            )
          }
        }
  ...
  fun workspaceDirectories(project: Project): WorkspaceDirectoriesResult {
    ...
    val directoriesToExclude =
        directoriesSection.excludedValues + symlinksToExclude + additionalDirectoriesToExclude
  ...
  private fun computeSymlinksToExclude(workspaceRoot: URI): List<Path> {
    ...
    val symlinks =
        (stableSymlinkNames + workspaceSymlinkNames).map { workspaceRootPath.resolve(it) }
    val realPaths =
        symlinks.mapNotNull {
          try {
            it.toRealPath()
  ...
  private fun Path.toDirectoryItem() = DirectoryItem(uri = this.toUri().toString())

  private fun toBuildTarget(module: Module): BuildTarget {
    val label = BspMappings.toBspId(module)
    val dependencies = module.directDependencies.map(BspMappings::toBspId)
    ...
    val buildTarget = BuildTarget(label, tags, languages, dependencies, capabilities)
  ...
  fun sources(project: Project, sourcesParams: SourcesParams): SourcesResult {
    fun toSourcesItem(module: Module): SourcesItem {
      ...
      val sourceItems =
          sourceSet.sources.map { SourceItem(BspMappings.toBspUri(it), SourceItemKind.FILE, false) }
      val generatedSourceItems =
          sourceSet.generatedSources.map {
            SourceItem(BspMappings.toBspUri(it), SourceItemKind.FILE, true)
          }
    ...
    val sourcesItems =
        labels.map { project.findModule(it)?.let(::toSourcesItem) ?: emptySourcesItem(it) }
  ...
  fun resources(project: Project, resourcesParams: ResourcesParams): ResourcesResult {
    ...
    val resourcesItems =
        labels.map { project.findModule(it)?.let(::toResourcesItem) ?: emptyResourcesItem(it) }
  ...
  fun inverseSources(
    ...
    val documentRelativePath =
        documentUri.toPath().relativeToOrNull(project.workspaceRoot.toPath())
            ?: throw RuntimeException("File path outside of project root")
    return InverseSourcesQuery.inverseSourcesQuery(
        documentRelativePath, bazelRunner, project.bazelRelease, cancelChecker)
  }
  ...
  fun dependencySources(
    ...
    fun getDependencySourcesItem(label: Label): DependencySourcesItem {
      val sources =
          project.findModule(label)?.sourceDependencies?.map(BspMappings::toBspUri).orEmpty()
    ...
    fun getItem(label: Label): OutputPathsItem {
      val items =
          project
              ...
              ?.let { module ->
                module.outputs.map {
                  OutputPathItem(BspMappings.toBspUri(it), OutputPathItemKind.DIRECTORY)
                }
              }
              .orEmpty()
  ...
  fun jvmCompileClasspath(
    ...
    val items =
        params.targets.collectClasspathForTargetsAndApply(project, true, cancelChecker) {
            module,
            ideClasspath ->
  ...
  private fun getJvmEnvironmentItems(
    ...
    fun extractJvmEnvironmentItem(
        ...
        module.javaModule?.let { javaModule ->
          JvmEnvironmentItem(
              ...
              .apply {
                mainClasses =
                    javaModule.mainClass
                        ?.let { listOf(JvmMainClass(it, javaModule.args)) }
                        .orEmpty()
              }
        }
    ...
    return targets.mapNotNull {
      ...
      val cqueryResult =
          ClasspathQuery.classPathQuery(it, cancelChecker, bspInfo, bazelRunner).runtime_classpath
  ...
  fun buildTargetJavacOptions(
    ...
    val items =
        params.targets.collectClasspathForTargetsAndApply(
            project, includeClasspath, cancelChecker) { module, ideClasspath ->
  ...
  private fun <T> List<BuildTargetIdentifier>.collectClasspathForTargetsAndApply(
      ...
      this.mapNotNull { project.findModule(Label.parse(it.uri)) }
          .mapNotNull {
            val classpath =
                if (includeClasspath) readIdeClasspath(it.label, cancelChecker) else emptyList()
            mapper(it, classpath)
          }
  ...
  private fun readIdeClasspath(targetLabel: Label, cancelChecker: CancelChecker): List<URI> {
    ...
    val classPathFromQuery =
        ClasspathQuery.classPathQuery(targetIdentifier, cancelChecker, bspInfo, bazelRunner)
    val ideClasspath =
  ...
  private fun resolveClasspath(cqueryResult: List<String>) =
      cqueryResult
          ...
          .filter {
            it.toFile().exists()
          } // I'm surprised this is needed, but we literally test it in e2e tests
          .map { it.toUri() }
  ...
  fun buildDependencyModules(
    ...
    val dependencyModulesItems =
        project.modules
            .filter { targetSet.contains(BuildTargetIdentifier(it.label.value)) }
            .map { module ->
              ...
              val moduleItems =
                  DependencyMapper.allModuleDependencies(project, module).flatMap { libraryDep ->
                    if (libraryDep.outputs.isNotEmpty()) {
                      val mavenDependencyModule =
                          DependencyMapper.extractMavenDependencyInfo(libraryDep)
                      val dependencyModule =
                          DependencyModule(
                              libraryDep.label.value, mavenDependencyModule?.version ?: "")
  ...
  fun rustWorkspace(project: Project, params: RustWorkspaceParams): RustWorkspaceResult {
    ...
    val requestedModules =
        BspMappings.getModules(project, params.targets).filter { Language.RUST in it.languages }
