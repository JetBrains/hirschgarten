...
class JavaLanguagePlugin(
  ...
  override fun prepareSync(targets: Sequence<TargetInfo>) {
    val ideJavaHomeOverride =
        workspaceContextProvider.currentWorkspaceContext().ideJavaHomeOverrideSpec.value
    jdk =
        ideJavaHomeOverride?.let { Jdk(version = "ideJavaHomeOverride", javaHome = it.toUri()) }
            ?: jdkResolver.resolve(targets)
  }
  ...
  override fun resolveModule(targetInfo: TargetInfo): JavaModule? =
      targetInfo.takeIf(TargetInfo::hasJvmTargetInfo)?.jvmTargetInfo?.run {
        ...
        val binaryOutputs =
            jarsList.flatMap { it.binaryJarsList }.map(bazelPathsResolver::resolveUri)
  ...
  private fun javaVersionFromJavacOpts(javacOpts: List<String>): String? =
      javacOpts.firstNotNullOfOrNull {
        ...
        if (flagName == "-target" || flagName == "--target" || flagName == "--release") argument
        else null
      }
