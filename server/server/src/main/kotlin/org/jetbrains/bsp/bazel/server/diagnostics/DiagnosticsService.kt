...
/**
 * @property hasAnyProblems keeps track of problems in given file so BSP reporter can publish
 *   diagnostics with an empty array, to clear up former diagnostics. see:
...
class DiagnosticsService(
  ...
  fun clearFormerDiagnostics(targetLabel: Label): List<PublishDiagnosticsParams> {
    ...
    return toClear
        ?.map {
          PublishDiagnosticsParams(it, BuildTargetIdentifier(targetLabel.value), emptyList(), true)
        }
        .orEmpty()
  }
