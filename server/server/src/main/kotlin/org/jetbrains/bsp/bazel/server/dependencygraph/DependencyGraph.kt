...
class DependencyGraph(
  ...
  init {
    idToDirectDependenciesIds =
        idToTargetInfo.entries.associate { (id, target) -> Pair(id, getDependencies(target)) }
  ...
  ): Lazy<Set<TargetInfo>> = lazy { calculateTransitiveDependenciesForTarget(targetInfo) }
  ...
  private fun calculateStrictlyTransitiveDependencies(dependencies: Set<Label>): Set<TargetInfo> =
      dependencies.flatMap { idToLazyTransitiveDependencies[it]?.value.orEmpty() }.toSet()
  ...
  private fun directDependenciesIds(targetIds: Set<Label>) =
      targetIds.flatMap { idToDirectDependenciesIds[it].orEmpty() }.toSet()
