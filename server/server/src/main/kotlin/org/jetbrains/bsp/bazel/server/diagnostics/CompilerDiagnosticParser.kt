...
object CompilerDiagnosticParser : Parser {
  ...
  // Example:
  // server/DiagnosticsServiceTest.kt:12:18: error: type mismatch: inferred type is String but Int
  // was expected
  ...
  fun tryParseOne(output: Output): Diagnostic? =
      output.tryTake(DiagnosticHeader)?.let { match ->
  ...
  private val IssueDetails =
      """^\s+.*|${IssuePositionMarker.pattern}""".toRegex() // indented line or ^
