...
class ExecuteService(
  ...
  private fun <T> withBepServer(
    ...
    val server =
        BepServer(
            compilationManager.client, diagnosticsService, originId, target, bazelPathsResolver)
  ...
  fun test(cancelChecker: CancelChecker, params: TestParams): TestResult {
    ...
    try {
      if (params.dataKind == BazelTestParamsData.DATA_KIND) {
        bazelTestParamsData =
            gson.fromJson(params.data as JsonObject, BazelTestParamsData::class.java)
      }
    } catch (e: Exception) {
  ...
  ): List<BuildTargetIdentifier> = selectModules(cancelChecker, targets).map { toBspId(it) }

  private fun selectModules(
      cancelChecker: CancelChecker,
      targets: List<BuildTargetIdentifier>
  ): List<Module> {
...
private fun throwResponseError(message: String, data: Any? = null): Nothing =
    throw ResponseErrorException(ResponseError(ResponseErrorCode.InvalidRequest, message, data))
