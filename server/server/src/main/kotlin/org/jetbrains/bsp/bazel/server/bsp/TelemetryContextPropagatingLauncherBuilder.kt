...
class TelemetryContextPropagatingLauncherBuilder<T> : Launcher.Builder<T>() {
  override fun create(): Launcher<T> {
    ...
    val reader =
        TelemetryContextPropagatingStreamMessageProducer(input, jsonHandler, remoteEndpoint)
    ...
    val execService =
        if (executorService != null) executorService else Executors.newCachedThreadPool()
...
private class TelemetryContextPropagatingStreamMessageProducer(
  ...
  override fun handleMessage(input: InputStream?, headers: Headers?): Boolean {
    if (!openTelemetryInitialized) {
      // The first call via BSP is build/initialize, at which point the telemetry isn't initialized
      // yet.
      return super.handleMessage(input, headers)
    }
    val context =
        ExtendedContextPropagators.extractTextMapPropagationContext(
                currentHeaders, openTelemetry.propagators)
