...
class RustLanguagePlugin(private val bazelPathsResolver: BazelPathsResolver) :
    LanguagePlugin<RustModule>() {
  ...
  private fun resolveTargetCrateRoot(
      rustCrateInfo: RustCrateInfo,
      location: RustCrateLocation
  ): String {
    val path =
  ...
  fun toRustWorkspaceResult(
      requestTargets: List<Module>,
    ...
    return RustWorkspaceResult(packages, rawDependencies, dependencies, resolvedTargets)
  }

  /**
   * Find all targets that are needed to be resolved. It includes all targets from `targets` and all
   * their dependencies.
   *
   * Let's assume we have a following project: B A | / \ C D E \ / | F G We want to resolve all
   * targets related to B. We only need B, C and F.
   *
