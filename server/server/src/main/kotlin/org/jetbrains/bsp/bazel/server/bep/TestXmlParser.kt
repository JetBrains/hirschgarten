...
data class TestSuite(
    ...
    @JacksonXmlProperty(isAttribute = true) val errors: Int,
    @JacksonXmlProperty(isAttribute = true) val time: Double,
    @JacksonXmlProperty(isAttribute = true) val id: Int,
    @JacksonXmlElementWrapper(useWrapping = false)
    @JacksonXmlProperty(localName = "testcase")
    val testcase: List<TestCase> = emptyList(),
    @JacksonXmlProperty(isAttribute = true, localName = "package") val pkg: String?,
...
data class TestCase(
    ...
    @JacksonXmlProperty(localName = "error") val error: TestResultDetail? = null,
    @JacksonXmlProperty(localName = "failure") val failure: TestResultDetail? = null,
    @JacksonXmlProperty(localName = "skipped") val skipped: TestResultDetail? = null
)
...
class TestResultDetail() {
  ...
  @JacksonXmlProperty(isAttribute = true) var message: String? = null

  // Error type information.
  // This typically gives the class name of the error, but may be absent or used for a similar
  // alternative value.
  @JacksonXmlProperty(isAttribute = true) var type: String? = null

  // Content between the tags, which typically includes the full error stack trace.
  @JacksonXmlText(value = true) @JacksonXmlProperty(localName = "") var content: String? = null
}
...
class TestXmlParser(
  ...
  /**
   * Processes a test result xml file, reporting suite and test case results as task start and
   * finish notifications. Parent-child relationship is identified within each suite based on the
   * TaskId.
   *
  ...
  fun parseAndReport(testXmlUri: String) {
    ...
    testSuites.testsuite.forEach { suite -> processSuite(suite) }
  }
   ...
   * The parents field in each notification's TaskId will be used to indicate the parent-child
   * relationship.
   *
  ...
  private fun processSuite(suite: TestSuite) {
    ...
    val suiteData =
        JUnitStyleTestCaseData(suite.time, null, suite.pkg, suite.systemErr.toString(), null)
    val suiteStatus =
        when {
    ...
    suite.testcase.forEach { case -> processTestCase(suite, suiteTaskId.id, case) }
  ...
  /**
   * Convert a TestCase into a taskStart and taskFinish notification to the client. The test case
   * will be associated with its parent suite.
   *
   * @param parentSuite TestSuite to which this test case belongs.
   * @param parentId String identifying the parent's TaskId. Used to indicate the proper tree
   *   structure.
  ...
  private fun processTestCase(parentSuite: TestSuite, parentId: String, testCase: TestCase) {
    ...
    val testCaseData =
        JUnitStyleTestCaseData(
            testCase.time, testCase.classname, parentSuite.pkg, fullOutput, errorType)
