...
class RustPackageResolver(val bazelPathsResolver: BazelPathsResolver) {
  ...
  fun rustPackages(rustBspTargets: List<Module>): List<RustPackage> =
      rustBspTargets.groupBy { resolvePackage(it).packageName }.mapNotNull(::resolveSinglePackage)
  ...
  private fun removeConflictingRustTargets(
      ...
      allRustTargets
          ...
          .map { (_, targets) -> mergeTargetsWithSameCrateRoot(targets) }
  ...
  private fun resolvePackageRootUrl(rustTargets: List<RustTargetModule>): String =
      rustTargets.map { (genericData, _) -> genericData.baseDirectory.toString() }.firstOrNull()
          ?: "Missing package root path"
  ...
  private fun resolvePackageFeatures(
    ...
    val allFeaturesAsStrings = rustTargets.flatMap { (_, rustData) -> rustData.crateFeatures }
