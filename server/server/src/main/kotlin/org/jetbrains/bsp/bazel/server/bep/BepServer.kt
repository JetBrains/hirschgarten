...
class BepServer(
  ...
  private fun processTestResult(event: BuildEventStreamProtos.BuildEvent) {
    if (event.hasTestResult()) {
      ...
      // TODO: this is the place where we could parse the test result and produce individual test
      // events
      ...
      val testReport = TestReport(target, passed, failed, ignored, cancelled, skipped)
  ...
  private fun fetchNamedSet(event: BuildEventStreamProtos.BuildEvent) {
    if (event.id.hasNamedSet()) {
      bepOutputBuilder.storeNamedSet(event.id.namedSet.id, event.namedSetOfFiles)
    }
  }
  ...
  private fun consumeUnsuccessfulActionCompletedEvent(
      actionEvent: BuildEventStreamProtos.ActionExecuted,
      label: String
  ) {
  ...
  private fun consumeCompletedEvent(event: BuildEventStreamProtos.BuildEvent) {
     ...
     * */
    val labelText =
        if (target != null && ("@$eventLabel" == target.uri || "@@$eventLabel" == target.uri))
            target.uri
        else eventLabel
    ...
    val shouldClearDiagnostics =
        outputGroups.stream().anyMatch { group: BuildEventStreamProtos.OutputGroup ->
          group.name == "default"
        }
  ...
  private fun consumeAbortedEvent(aborted: BuildEventStreamProtos.Aborted) {
    if (aborted.reason != BuildEventStreamProtos.Aborted.AbortReason.NO_BUILD) {
      LOGGER.warn("Command aborted with reason {}: {}", aborted.reason, aborted.description)
    }
  }
