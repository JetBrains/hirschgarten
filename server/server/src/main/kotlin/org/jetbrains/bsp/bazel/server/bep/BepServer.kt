...
class BepServer(
      ...
      if (coverageReportUri != null) {
        bspClient.onBuildPublishOutput(
          PublishOutputParams(
            originId,
            taskId,
            BuildTargetIdentifier(Label.parse(event.id.testResult.label).value),
  ...
  private fun consumeBuildStartedEvent(event: BuildEventStreamProtos.BuildEvent) {
    ...
    val target = BuildTargetIdentifier(Label.parse(event.id.testResult.label).value)
  ...
  private fun consumeCompletedEvent(event: BuildEventStreamProtos.BuildEvent) {
     ...
     * just use target.uri.
     * */
    val labelText = if ("@$eventLabel" == target.uri || "@@$eventLabel" == target.uri) target.uri else eventLabel
