...
class BazelBspServer(
  ...
  private fun bspServerData(
    ...
    val projectSyncService =
        ProjectSyncService(bspProjectMapper, projectProvider, telemetryConfigWithLogger)
    ...
    return BazelServices(serverLifetime, bspRequestsRunner, projectSyncService, executeService)
  }
  ...
  private fun createLanguagePluginsService(
    ...
    val javaLanguagePlugin =
        JavaLanguagePlugin(workspaceContextProvider, bazelPathsResolver, jdkResolver)
  ...
  private fun createProjectProvider(
    ...
    val bazelBspAspectsManager =
        BazelBspAspectsManager(
            bazelBspCompilationManager = compilationManager, aspectsResolver = aspectsResolver)
    val bazelBspLanguageExtensionsGenerator =
        BazelBspLanguageExtensionsGenerator(aspectsResolver, bazelInfo.release)
    val bazelBspFallbackAspectsManager =
        BazelBspFallbackAspectsManager(bazelRunner, workspaceContextProvider)
