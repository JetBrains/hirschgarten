...
class TargetInfoReader {
  fun readTargetMapFromAspectOutputs(files: Set<Path>): Map<Label, TargetInfo> {
    return runBlocking(Dispatchers.Default) {
        ...
        // this happens for all protobuf targets, for which a built-in aspect
        ...
        // entity-per-label. As long as we don't have it, in case of a conflict we just take the
        // entity
        // that contains JvmTargetInfo as currently it's the most important one for us. Later, we
        // sort by
        // shortest size to get a stable result, which should be the default config.
        .mapValues {
          it.value.filter(TargetInfo::hasJvmTargetInfo).minByOrNull { targetInfo ->
            targetInfo.serializedSize
          } ?: it.value.first()
        }
  ...
  private fun readFromFile(file: Path): TargetInfo {
    ...
    file.reader().use { parser.merge(it, builder) }
