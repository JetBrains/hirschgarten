...
fun createModule(
    ...
    Module(
        ...
        sourceSet =
            SourceSet(
                sources = sources, generatedSources = setOf<URI>(), sourceRoots = setOf<URI>()),
        resources = setOf<URI>(),
        outputs = setOf<URI>(),
...
fun getModuleWithoutDependencies(features: List<String> = emptyList()): Pair<RustModule, Module> {
  ...
  val module =
      createModule(
          ...
          sources = setOf<URI>(URI.create("file:///path/to/sample_target/src/lib.rs")),
...
fun getModulesWithDependency(): List<Module> {
  ...
  val rustModuleA =
      createRustModule(
          crateId = "dirA/src/lib.rs", crateRoot = "file:///path/to/targetA/src/lib.rs")
  val moduleA =
      createModule(
          label = "@//dirA:targetA",
          directDependencies = emptyList(),
...
fun getSampleModules(): Pair<List<Module>, Map<String, Module>> {
  ...
  val moduleA =
      createTarget(moduleName = "A", directDependencies = listOf("@//pkgD:D", "@//pkgE:E"))
  val moduleB =
      createTarget(moduleName = "B", directDependencies = listOf("@//pkgC:C", "@//pkgD:D"))
  val moduleC = createTarget(moduleName = "C", directDependencies = listOf("@//pkgF:F"))
  val moduleD =
      createTarget(moduleName = "D", directDependencies = listOf("@//pkgF:F", "@//pkgG:G"))
  val moduleE =
      createTarget(moduleName = "E", directDependencies = listOf("@//pkgG:G", "@//pkgH:H"))
  val moduleF = createTarget(moduleName = "F", directDependencies = emptyList())
  val moduleG = createTarget(moduleName = "G", directDependencies = emptyList())
  val moduleH = createTarget(moduleName = "H", directDependencies = emptyList())
