...
class BazelBspLanguageExtensionsGeneratorTest {
  ...
  internal class BazelExternalRulesQueryMock(private val ruleNames: List<String>) :
  ...
  fun `should create the extensions dot bzl file with one import and one toolchain (cpp)`() {
    // given
    val ruleLanguages = defaultRuleLanguages + listOf(RuleLanguage("rules_cc", Language.Cpp))
