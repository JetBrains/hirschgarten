...
class DependencyMapperTest {
  ...
  fun `should translate dependency`() {
    ...
    val expectedDependency =
        MavenDependencyModule(
            ...
            "scala-library",
            "2.13.11",
            listOf(expectedMavenArtifact, expectedMavenSourcesArtifact))
  ...
  fun `should not translate non maven dependency`() {
    val lib1 = Library(Label.parse("@//projects/v1:scheduler"), emptySet(), emptySet(), emptyList())
    val dependency = DependencyMapper.extractMavenDependencyInfo(lib1)
  ...
  fun `should gather deps transitively`() {
    ...
    val libraries =
        mapOf(lib1.label to lib1, lib2.label to lib2, lib3.label to lib3, lib4.label to lib4)
    val currentUri = Paths.get(".").toUri()
    val project =
        Project(currentUri, emptyList(), emptyMap(), libraries, emptyList(), BazelRelease(6))
    val module =
        Module(
            Label.parse(""),
            true,
