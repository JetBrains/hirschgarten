...
class RustDependencyResolverTest {
  ...
  fun `should return proper dependency graph for multiple targets with multiple dependencies`() {
    ...
    val dependenciesNames = dependencies.mapValues { (_, deps) -> deps.map { it.name } }
    ...
    val rawDependenciesNames = rawDependencies.mapValues { (_, deps) -> deps.map { it.name } }
    val trueRawDependenciesNames =
        trueDependenciesNames.mapValues { (_, names) -> names.map { "@//pkg$it:$it" } }
