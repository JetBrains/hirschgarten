...
class DiagnosticsServiceTest {
  ...
  fun `should parse kotlin errors`() {
    ...
    val diagnostics =
        extractDiagnostics(
            output, "//server/src/test/java/org/jetbrains/bsp/bazel/server/diagnostics:diagnostics")
        ...
        listOf(
            PublishDiagnosticsParams(
                ...
                BuildTargetIdentifier(
                    "//server/src/test/java/org/jetbrains/bsp/bazel/server/diagnostics:diagnostics"),
                ErrorDiagnostic(
                    Position(12, 18),
  ...
  fun `parse scala errors with warnings`() {
    ...
    // when
    val diagnostics =
        extractDiagnostics(output, "//project/src/main/scala/com/example/project:project")
    ...
    val expected =
        listOf(
            PublishDiagnosticsParams(
                TextDocumentIdentifier(
                    "file:///user/workspace/project/src/main/scala/com/example/project/File1.scala"),
            ...
            PublishDiagnosticsParams(
                TextDocumentIdentifier(
                    "file:///user/workspace/project/src/main/scala/com/example/project/File2.scala"),
                BuildTargetIdentifier("//project/src/main/scala/com/example/project:project"),
                WarningDiagnostic(
                    Position(26, 24),
  ...
  fun `parse another scala 3 multiple errors`() {
    ...
    val expected =
        listOf(
            PublishDiagnosticsParams(
                TextDocumentIdentifier(
                    "file:///user/workspace/drd/messaging/src/main/scala/bots/Bot.scala"),
                BuildTargetIdentifier("//project/src/main/scala/com/example/project:project"),
                ErrorDiagnostic(
                    Position(143, 26),
                    """|Not Found Error
  ...
  fun `parse scala warnings`() {
    ...
    val diagnostics =
        extractDiagnostics(
            output, "//intellij/release-tool/src/main/scala/com/intellij/releasetool:releasetool")
    ...
    val expected =
        listOf(
            PublishDiagnosticsParams(
                ...
                BuildTargetIdentifier(
                    "//intellij/release-tool/src/main/scala/com/intellij/releasetool:releasetool"),
                WarningDiagnostic(
                    Position(14, 5),
  ...
  fun `should parse java symbol not found error`() {
    ...
    val expected =
        listOf(
            PublishDiagnosticsParams(
                TextDocumentIdentifier(
                    "file:///user/workspace/server/src/main/java/org/jetbrains/bsp/bazel/server/bep/BepServer.java"),
                BuildTargetIdentifier("//project/src/main/scala/com/example/project:project"),
                ErrorDiagnostic(
                    Position(55, 34),
                    """
  ...
  fun `should clear former diagnostics`() {
    ...
    val diagnosticsBeforeError =
        service.clearFormerDiagnostics(Label.parse("//path/to/package:test"))
    diagnosticsBeforeError.shouldBeEmpty()

    // Extract the diagnostics from BEP, there's an error in `Test.scala` of
  ...
  private fun PublishDiagnosticsParams(
    ...
    return PublishDiagnosticsParams(textDocument, buildTarget, diagnostics.asList(), true)
  }
  ...
  private fun createDiagnostic(
    ...
    return BspDiagnostic(Range(adjustedPosition, adjustedPosition), message).apply {
  ...
  private fun extractDiagnostics(
    ...
    return DiagnosticsService(workspacePath, ConcurrentHashMap())
        .extractDiagnostics(output, buildTarget, null, false)
  }
}
