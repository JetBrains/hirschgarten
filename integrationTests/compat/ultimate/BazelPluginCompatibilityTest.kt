...
class BazelPluginCompatibilityTest {
  ...
  private suspend fun buildBazelPlugin(): BazelPluginInfo {
    ...
    val properties = IdeaUltimateProperties(ULTIMATE_HOME)
    properties.productLayout.pluginLayouts.patchPluginXml(
      plugins = listOf(BAZEL_PLUGIN_MODULE),
    ...
    CompilationTasks.create(context).compileModules(listOf(BAZEL_PLUGIN_MODULE))
    createBuildTasks(context).buildNonBundledPlugins(listOf(BAZEL_PLUGIN_MODULE))
